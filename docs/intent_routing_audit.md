...
## Addendum — 2026-05-30: continuous intent measurement (launch hardening, Phase 2)

The 2026-05-15 audit was a one-time manual pass. To make routing quality
**measurable and regression-proof** going forward, the deterministic layer now
has a model-free eval + conflict harness wired into CI.

### Tooling

- **`scripts/diagnostics/intent_eval.py`** — runs a golden corpus
  (`tests/intent_corpus/*.yaml`, ~100 cases across 13 domains) through
  `IntentRecognizer.plan()` and reports per-domain recall + negative-accuracy.
  CI gate: `tests/test_intent_eval.py`.
- **Conflict / overlap detector** (`--conflicts`) — runs every parser
  independently via the new shared `IntentRecognizer._clause_parsers()` and
  flags (a) utterances matched by more than one parser and (b) any `not:`
  negative produced by *some* parser (latent poaching that ordering masks).
  CI gate: `tests/test_intent_conflicts.py`.
- **`scripts/diagnostics/routing_stats.py`** — summarizes the live `[ROUTE]`
  decision log (source distribution, fallback rate, latency p50/p95, tool mix).

### Findings & fixes (all verified by the corpus, which fails on pristine HEAD)

| Utterance | Was | Now | Cause |
|---|---|---|---|
| "end the focus session" | `start_focus_session` | `end_focus_session` | end regex allowed only `my`, not `the` → fell to the bare `focus session` start branch |
| "focus session status" | `start_focus_session` | `focus_session_status` | status regex didn't allow the `session` noun between `focus` and `status` |
| "take a note" | `start_dictation` | `save_note` | dictation start regex claimed a bare `note`; now requires `note taking` |
| "what are my notes" / "show me my notes" | (no route) | `read_notes` | added interrogative + filler-`me` phrasings |
| "export my memories" | (no route) | `export_memory` | regex required singular `memory`; now `memor(y|ies)` |
| "how much ram am I using" | (no route) | `get_cpu_ram` | added `how much <cpu/ram/memory> am I using` phrasing |
| "is it going to rain today" | (no route) | `get_weather` | added `going to rain/snow` |
| "give me a literature review of X" | (no route) | `research_topic` | literature-review pattern now accepts a `give me a …` prefix |

### Current state

- **Overlaps:** the only multi-matcher utterances are the *documented, intentional*
  `search_indexed_files` ⇄ `search_file` split for explicit filenames (the index
  lookup wins). Pinned by `KNOWN_OVERLAP_TOOLSETS`.
- **Latent poaching:** none.
- **Calibrated confidence:** `IntentEngine` now reads an optional per-action
  `confidence` (default 1.0), so a parser can route a genuinely ambiguous match
  into the existing confirmation band instead of dispatching silently.
