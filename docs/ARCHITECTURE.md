...
## Launch architecture overview (2026-06-01)

> **Read this first.** The sections below (§0 onward) are the deep, dated
> reference. This overview is the curated map for new readers: the turn
> lifecycle, the **routing pipeline with its confidence bands**, and the
> **workflow state machine**. For the routing layer in depth see
> [intent_recognition.md](intent_recognition.md); for every tunable knob see
> [config_reference.md](config_reference.md).

### A. Turn lifecycle

A turn — whether it arrives as voice or text — flows through one pipeline:
...
voice / text in
   ...
   STTEngine ──► TurnOrchestrator (v2, core/planning/)
                     │
            _build_context_bundle()      (persona + session summary + memory recall)
                     │
              PlannerEngine.plan()  ◄── the 5-layer router (§B)
                     │
                 ToolPlan  (mode: tool | planner | chat | reply)
                     │
        OrderedToolExecutor  ⇄  GraphCompiler (LangGraph, opt-in via routing.execution_engine)
                     │
            CapabilityExecutor ──► capability handler (28 plugin modules)
                     │
            ResponseFinalizer ──► persona styling ──► TTS / GUI / channel
                     │
       MemoryCuratorAgent.curate()  (writes the turn to the domain stores + Chroma)
```

`TurnOrchestrator` owns the lifecycle; `PlannerEngine` (`core/planning/planner_engine.py`)
owns route selection; `CapabilityExecutor` enforces the lock gate and online-consent
gate before any handler runs.

### B. The routing pipeline — 5 layers, cheapest first

`PlannerEngine.plan()` walks an ordered chain and returns at the **first layer
that produces a plan**. Each layer is more expensive than the last, so the
common phrasings never pay for the LLM:
...
```
 ...
 │ L5  llm_chat           conversational fallback                            │  chat
 └─────────────────────────────────────────────────────────────────────────┘
```

Concretely, in `PlannerEngine.plan()` order: pending-online → workflow-continue →
`IntentRecognizer` multi-action → `_find_best_route` (RouteScorer) → learned
dispatch → lexical → **if planner enabled** QwenPlanner → **else** mid-confidence
embedding-confirm → chat.

A regex hit at L1 with confidence ≥ `IntentEngine.HIGH_THRESHOLD` (**0.9**)
short-circuits everything else and builds the plan directly from the
`IntentResult` — this is why every capability needs a deterministic pattern
(see [intent_recognition.md](intent_recognition.md)).

### C. Confidence bands

The fuzzy layers don't dispatch on a bare best-match; each defends a band so a
near-miss asks rather than silently guessing. Defaults (all tunable under
`routing.*` — see [config_reference.md](config_reference.md)):

| Layer | Knob | Default | Meaning |
|---|---|---|---|
| Intent fast-path | `IntentEngine.HIGH_THRESHOLD` | `0.90` | At/above → bypass the planner, build plan from regex result. |
| Intent fast-path | `IntentEngine.MEDIUM_THRESHOLD` | `0.50` | Below HIGH but here → keep the candidate, still consult planner. |
| Embedding | `routing.dispatch_threshold` | `0.62` | Cosine **≥** this → auto-dispatch the matched capability. |
| Embedding | `routing.confirm_low` | `0.50` | Cosine in **[confirm_low, dispatch_threshold)** → ask *"did you mean …?"* instead of dropping to chat. |
| Embedding | `routing.tie_epsilon` | `0.05` | Two candidates within this of the top → treat as a tie and disambiguate. |
| Lexical | `routing.lexical_threshold` | `88` | rapidfuzz score must clear this to fire. |
| Lexical | `routing.lexical_margin` | `6` | …**and** beat the runner-up tool by this margin (never poaches). |
| Learned | `routing.promote_after` | `3` | A phrasing confirmed this many times is promoted to deterministic dispatch. |

The mid-confidence embedding **confirm band** is the key safety property: rather
than letting the small chat model fabricate a plausible success for a capability
it didn't actually call, FRIDAY asks a yes/no question — and that answer becomes
the day-by-day learning signal that feeds the `promote_after` counter.

### D. Workflow state machine

Anything that can't resolve in one turn runs as a **workflow** through
`WorkflowOrchestrator` (`core/workflow_orchestrator.py`). A workflow persists its
state in `session_state` so the **next** turn resumes mid-flow (pre-check 2
above). Three reusable, composable mechanisms cover the common multi-turn shapes
— all are pure session-state + regex (no platform branch), gated by config:

```
              ┌──────────────── handler resolves a request ───────────────┐
              │                          │                                 │
       >1 candidate?                destructive?                    needs a slot?
              │                          │                                 │
   DisambiguationGuard.arm()    ConfirmationGuard.arm()        start_template_slot_fill()
   pending_pick + numbered      pending_destructive +          TemplateWorkflow
   list                         specific preview               (ask: / slot: steps)
              │                          │                                 │
   next turn → _parse_pending_pick   → _parse_pending_destructive   → run_slot_fill_turn
   (selection-shaped only;        (affirmation → confirm,         (extractor → LLM →
    else falls through)            else cancel)                    default precedence)
              │                          │                                 │
   pick() fills arg, re-dispatch  confirm() re-dispatches with   schedule step runs the
   stored action (_picked=True)   _confirmed=True                wrapped capability
```

- **ConfirmationGuard** (`core/workflows/confirmation.py`) — *"shall I go
  ahead?"* before a destructive action. Wired into `lock_screen`,
  `delete_goal`, `shutdown_assistant`, `forget_memory`, and Home Assistant
  on/off, plus a memory-wipe preview. Gate: `routing.confirm_destructive`
  (default true). The explicit `/lock` slash bypasses the guard.
- **DisambiguationGuard** (`core/workflows/disambiguation.py`) — *"which one
  did you mean?"* when a request resolves to more than one candidate. Wired
  into `search_indexed_files`, `launch_app`, and `query_document`. Gate:
  `routing.disambiguate` (default true). A non-selection reply falls through to
  normal routing, so the user is never trapped.
- **Slot-fill templates** — declarative YAML (`core/workflows/templates/`)
  driven by `SlotFiller` (`core/planning/slot_filling.py`) with cheapest-first
  precedence: caller-known → deterministic extractor → LLM (only for still-missing
  required slots) → default. The live example is `set_reminder.yaml` (two-phase
  date → time).

Both guards share one interceptor convention: their `_parse_pending_*` parser
sits at the **top** of the IntentRecognizer chain so the follow-up turn is
caught before any normal parser, and each guard's own `confirm`/`pick`/`cancel`
is the single owner of clearing the pending flag (it can't go stale).
...
> project (repository root `Friday_Linux/`). Every Python file in the project is described.
...
│   ├── stores/                 SQLite + ChromaDB persistence (domain stores;
│   │                           ContextStore facade in stores/context_store.py)
...
**Path:** `core/intent_recognizer.py`
...
**Path:** `core/capability_registry.py`
...
**Path:** `core/capability_broker.py`
...
**Path:** `core/assistant_context.py`
...
**Path:** `core/conversation_agent.py`
...
**Path:** `core/memory/procedural.py`
...
**Path:** `core/memory/embeddings.py`
...
**Path:** `core/stores/` (facade: `core/stores/context_store.py`)

> **Track 5.1 decomposition (2026-05-19).** The original 1480-line god-class
> `core/context_store.py` was split into a set of **domain stores** under
> `core/stores/`, each owning ≤4 tables plus its own migration SQL under
> `core/stores/migrations/`. `ContextStore` survives only as a **transitional
> facade** (no own state) that delegates to the domain stores; new code prefers
> the direct stores (`app.session_store`, `app.memory_store`, …). Write-ownership
> is strict (each store creates and writes only its own tables); reads may cross
> stores via raw SQL on the shared `data/friday.db`.

### Domain stores and their tables

| Store | File | Tables |
|---|---|---|
| Session | `core/stores/session_store.py` | `sessions`, `turns`, `personas`, `conversation_sessions` |
| Memory | `core/stores/memory_store.py` | `facts`, `memory_items` (+ owns the Chroma vector index) |
| Knowledge graph | `core/stores/knowledge_graph_store.py` | `entities`, `entity_facts`, `entity_relationships` |
| Audit | `core/stores/audit_store.py` | `audit_events`, `online_permission_events`, `agent_messages`, `commitments` |
| Goal | `core/stores/goal_store.py` | `goals`, `goal_progress` |
| Workflow | `core/stores/workflow_store.py` | `workflows` |
| Intent learning | `core/stores/intent_learning_store.py` | `routing_observations`, `learned_phrases`, `intent_profile` |
| File index | `core/stores/file_index_store.py` | `file_index` |
| App index | `core/stores/app_index_store.py` | `app_index` |

`WorkingArtifact` and `ARTIFACT_SCOPE_RANK` live in `session_store.py` (re-exported
from `core.stores`). SQLite runs in **WAL mode** for better concurrent read performance.

> **Reminders / notes are not in `ContextStore`.** `TaskManagerPlugin`
> (`modules/task_manager/plugin.py`) keeps its **own** SQLite database with `notes`
> and `calendar_events` tables. Reminders are rows in `calendar_events` tagged with
> a `type` column (`'reminder'` by default). Calendar *events* proper are owned by
> the Google Calendar (WorkspaceAgent) path — the local calendar-event capabilities
> were removed 2026-05-31.
...
**Path:** `modules/voice_io/wake_detector.py`
...
**Path:** `modules/voice_io/clap_detector.py`
...
**Path:** `core/speech_coordinator.py`
...
**Path:** `core/extensions/loader.py`
...
**Path:** `core/extensions/protocol.py`
...
**Path:** `core/extensions/decorators.py`
...
**Path:** `core/workflow_orchestrator.py`
...
**Path:** `core/reasoning/workflows/focus_mode.py`
...
**Path:** `core/reasoning/route_scorer.py`
...
**Path:** `core/reasoning/model_router.py`
...
**Path:** `core/reasoning/graph_compiler.py`
...
**Path:** `core/kernel/permissions.py`
...
**Path:** `core/bootstrap/container.py`
...
**Path:** `core/bootstrap/lifecycle.py`
...
**Path:** `core/bootstrap/settings.py`
...
**Path:** `modules/browser_automation/`
...
**Path:** `modules/task_manager/plugin.py`
...
**Path:** `modules/llm_chat/plugin.py`
...
**Path:** `modules/greeter/extension.py`
...
**Path:** `modules/focus_session/plugin.py`
...
**Path:** `core/response_finalizer.py`
...
**Path:** `core/system_capabilities.py`
...
flowchart TD
    ...
    APP --> CS[core/stores/ ContextStore facade]
...
| `core/stores/` | Domain stores (`session_store`, `memory_store`, `goal_store`, …) — SQLite WAL + ChromaDB + `HashEmbeddingFunction`; `ContextStore` is the transitional facade in `stores/context_store.py` (see §15) |
