...
``core/`` tree. Logs to the terminal AND to a rotating file (``logs/friday.log``
by default) so there's always a trail to debug from.

Level resolution: ``FRIDAY_LOG_LEVEL`` env var > config ``logging.level`` >
...
def configure_logging(level: str | None = None, log_file: str | None = "logs/friday.log",
                      to_file: bool = True) -> logging.Logger:
    """Set the level and attach console + rotating-file handlers (idempotent)."""
    lvl = (level or os.environ.get("FRIDAY_LOG_LEVEL", "INFO")).upper()
...
if not logger.handlers:
    ...
    logger.setLevel(getattr(logging, os.environ.get("FRIDAY_LOG_LEVEL", "INFO").upper(), logging.INFO))
    logger.propagate = False
