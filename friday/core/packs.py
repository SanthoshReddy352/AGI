...
FRIDAY can author its own **skills** (``~/.friday/skills/<slug>/SKILL.md``) and
**tools** (``~/.friday/tools/*.py``). A *pack* is a single ``.zip`` that bundles a
    ...
    *,
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
        ...
        for t in manifest.get("tools", []):
            ...
            try:
                from friday.tools.authoring import _load_user_tool
