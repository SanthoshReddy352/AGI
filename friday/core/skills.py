...
Skills live under two roots:
  ...
  * **user / learned** — ``~/.friday/skills/`` (where ``create_skill`` writes;
...
class SkillStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.user_dir = Path(user_dir or "~/.friday/skills").expanduser()
    ...
    def catalog_text(self, limit: int = 60) -> str:
        skills = self.all()[:limit]
        if not skills:
            return ""
