...
export default function LearningDetailView() {
  ...
  const [view, setView] = useState(() => localStorage.getItem("friday-path-view") || "list");
  useEffect(() => { localStorage.setItem("friday-path-view", view); }, [view]);
    ...
    <>
      ...
      <main className="flex-1 overflow-y-auto px-6 py-6">
        <div className="max-w-5xl mx-auto grid lg:grid-cols-[1fr_320px] gap-6">
            ...
            <div>
              <div className="flex items-end justify-between gap-3">
                <div className="min-w-0">
                  <h1 className="font-serif text-2xl truncate">{topic.title}</h1>
                  <div className="text-[13px] text-ink-soft dark:text-night-faint capitalize">{topic.depth} depth</div>
                </div>
              ...
              {plan.length > 0 && (
                <div className="mt-3">
                  <div className="h-2 rounded-full bg-paper-sink dark:bg-night overflow-hidden">
                    <div className="h-full bg-brand rounded-full transition-all" style={{ width: `${pct}%` }} />
                  </div>
                  <div className="text-[12px] text-ink-faint dark:text-night-faint mt-1">{prog.done || 0} of {prog.total} modules complete</div>
                </div>
              )}
            </div>
          ...
          {/* Right: insights */}
          <Insights insights={insights} memory={memory}
...
function Insights({ insights, memory, preferences = [], onRemovePreference }) {
  ...
  return (
    <aside className="space-y-4 lg:sticky lg:top-0 self-start">
      {preferences.length > 0 && (
        <section className="rounded-2xl border border-line dark:border-night-line bg-paper-panel dark:bg-night-panel p-4">
          <h2 className="font-medium mb-1.5">Teaching preferences</h2>
          <p className="text-[12px] text-ink-faint dark:text-night-faint mb-2">
            Standing instructions you've set — applied in every module. Add more in the Path chat.
          </p>
          <ul className="space-y-1.5">
            {preferences.map((p, i) => (
              <li key={i} className="group flex items-start gap-2 rounded-lg px-2.5 py-1.5 bg-paper-soft dark:bg-night text-[12.5px]">
                <span className="flex-1">{p}</span>
                <button onClick={() => onRemovePreference?.(i)} title="Remove preference"
                        className="opacity-0 group-hover:opacity-100 text-ink-faint hover:text-brand-deep shrink-0">
      ...
      <section className="rounded-2xl border border-line dark:border-night-line bg-paper-panel dark:bg-night-panel p-4">
        <h2 className="font-medium mb-3">Understanding</h2>
        <div className="flex items-center gap-3">
          <ScoreRing value={score} />
          <div className="text-[13px] text-ink-soft dark:text-night-faint">
            {score == null ? "I'll gauge this as we go and as you answer checks."
              : score >= 75 ? "Strong grasp so far." : score >= 45 ? "Getting there — a few gaps." : "Early days — we'll build it up."}
          </div>
        </div>
        {insights.analysis && (
          <p className="mt-3 text-[13px] text-ink-soft dark:text-night-ink border-t border-line dark:border-night-line pt-3">{insights.analysis}</p>
        )}
        {(insights.strengths?.length > 0 || insights.gaps?.length > 0) && (
          <div className="mt-3 grid grid-cols-2 gap-2 text-[12px]">
            <Chips label="Strengths" items={insights.strengths} tone="emerald" />
            <Chips label="Gaps" items={insights.gaps} tone="brand" />
          </div>
        )}
      </section>

      <section className="rounded-2xl border border-line dark:border-night-line bg-paper-panel dark:bg-night-panel p-4">
        <h2 className="font-medium mb-2">Quiz checks</h2>
        {quiz.total ? (
          <>
            <div className="text-[13px] text-ink-soft dark:text-night-faint mb-2">{quiz.correct}/{quiz.total} correct ({quiz.score}%)</div>
            <div className="space-y-1.5 max-h-64 overflow-y-auto">
              {(quiz.items || []).slice().reverse().map((q, i) => (
                <QuizHistoryItem key={i} q={q} />
              ))}
            </div>
          </>
        ) : <div className="text-[13px] text-ink-faint dark:text-night-faint">No checks yet.</div>}
      </section>

      <section className="rounded-2xl border border-line dark:border-night-line bg-paper-panel dark:bg-night-panel p-4">
        <h2 className="font-medium mb-2">Visuals & demos</h2>
        {artifacts.length ? (
          <div className="grid grid-cols-2 gap-2">
            {artifacts.map((a, i) => (
              <a key={i} href={a.url} target="_blank" rel="noreferrer noopener"
                 className="block rounded-lg border border-line dark:border-night-line overflow-hidden hover:shadow-soft transition" title={a.title || a.kind}>
                {a.kind === "diagram" || a.kind === "image" ? (
                  <img src={a.url} alt={a.title || a.kind} loading="lazy" className="w-full h-20 object-cover bg-white" />
                ) : (
                  <div className="h-20 grid place-items-center text-[12px] text-brand-deep bg-brand-wash dark:bg-night-soft">{a.kind}</div>
                )}
                <div className="px-2 py-1 text-[11px] truncate text-ink-soft dark:text-night-faint">{a.title || a.kind}</div>
              </a>
            ))}
          </div>
        ) : <div className="text-[13px] text-ink-faint dark:text-night-faint">No diagrams or demos yet.</div>}
      </section>

      {memory.length > 0 && (
        <section className="rounded-2xl border border-line dark:border-night-line bg-paper-panel dark:bg-night-panel p-4">
          <h2 className="font-medium mb-2">What I remember</h2>
          <ul className="space-y-1 text-[12.5px] text-ink-soft dark:text-night-faint list-disc pl-4">
            {memory.map((m) => <li key={m.id}>{m.content}</li>)}
          </ul>
        </section>
      )}
    </aside>
...
// One past check in the dashboard: click to expand the full question — every
// option (correct one ticked, a wrong pick crossed) plus the explanation.
function QuizHistoryItem({ q }) {
  const [open, setOpen] = useState(false);
  const options = q.options || [];
  const hasDetail = options.length > 0 || (q.user_answer || "").trim() || (q.explanation || "").trim();
  return (
    <div className="rounded-lg hover:bg-paper-soft dark:hover:bg-night transition">
      <button onClick={() => hasDetail && setOpen((o) => !o)}
              className={`w-full flex items-start gap-2 text-left text-[12.5px] px-1 py-1 ${hasDetail ? "cursor-pointer" : "cursor-default"}`}>
        <span className={`mt-0.5 h-4 w-4 shrink-0 grid place-items-center rounded-full text-white ${q.correct ? "bg-emerald-500" : "bg-brand"}`}>
          {q.correct ? "✓" : "×"}
        </span>
        <span className={`flex-1 text-ink-soft dark:text-night-faint ${open ? "" : "line-clamp-2"}`}>{q.question}</span>
        {hasDetail && (
          <svg className={`mt-1 shrink-0 text-ink-faint transition-transform ${open ? "rotate-180" : ""}`}
               width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m6 9 6 6 6-6" /></svg>
        )}
      </button>
      {open && (
        <div className="ml-6 mb-2 mr-1 space-y-1">
          {options.map((opt, j) => {
            const isAnswer = j === q.answer_index;
            const isPicked = j === q.picked_index || (q.picked_index == null && opt === q.user_answer);
            return (
              <div key={j} className={`flex items-center gap-1.5 text-[12px] rounded-md px-2 py-1 ${
                isAnswer ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300"
                  : isPicked ? "bg-brand-wash dark:bg-night-soft text-brand-deep"
                  : "text-ink-faint dark:text-night-faint"}`}>
                <span className="h-4 w-4 shrink-0 grid place-items-center rounded-full border border-current text-[10px]">
                  {String.fromCharCode(65 + j)}
                </span>
                <span className="flex-1">{opt}</span>
                {isAnswer && <span className="shrink-0">✓</span>}
                {isPicked && !isAnswer && <span className="shrink-0">— your pick</span>}
              </div>
            );
          })}
          {options.length === 0 && (q.user_answer || "").trim() && (
            <div className="text-[12px] text-ink-faint dark:text-night-faint">Your answer: {q.user_answer}</div>
          )}
          {(q.explanation || "").trim() && (
            <div className="text-[12px] text-ink-soft dark:text-night-faint border-l-2 border-line dark:border-night-line pl-2">
              {q.explanation}
            </div>
          )}
        </div>
  ...
  return (
    <div className="relative h-16 w-16 shrink-0">
      <svg viewBox="0 0 56 56" className="h-16 w-16 -rotate-90">
        <circle cx="28" cy="28" r={r} fill="none" strokeWidth="6" className="stroke-paper-sink dark:stroke-night" />
        <circle cx="28" cy="28" r={r} fill="none" strokeWidth="6" strokeLinecap="round"
...
function Chips({ label, items, tone }) {
  ...
  const cls = tone === "emerald"
    ...
    : "bg-brand-wash dark:bg-night-soft text-brand-deep";
  return (
    <div>
      <div className="text-ink-faint dark:text-night-faint mb-1">{label}</div>
      <div className="flex flex-wrap gap-1">
        {items.map((s, i) => <span key={i} className={`px-1.5 py-0.5 rounded ${cls}`}>{s}</span>)}
      </div>
    </div>
  );
}
