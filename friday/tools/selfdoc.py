...
# Standalone "FRIDAY" as the display name — but NOT the FRIDAY_* env-var prefix.
_NAME_TOKEN = re.compile(r"\bFRIDAY\b(?!_)")


def _about_friday(args: dict) -> ToolResult:
    ...
    # Reflect the configured display name (env-var names like FRIDAY_API_KEY stay).
    name = assistant_name()
    if name != "FRIDAY":
...
def register(registry: ToolRegistry) -> None:
    registry.register(
        "about_friday",
        "Look up how FRIDAY itself works/configures (modes, switching the model "
        "provider, tools, skills, memory, browser, telegram, logging). Use this to "
        "answer the user's questions about FRIDAY's own settings and capabilities.",
        {
            "type": "object",
            "properties": {
                "topic": {"type": "string",
