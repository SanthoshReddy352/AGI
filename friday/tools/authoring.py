"""Self-authoring — let FRIDAY write a brand-new tool on request and load it live.

The user can say "make a tool that …" and the agent calls ``create_tool`` with a
``run(args)`` function body. The module is written to ``~/.friday/tools/`` (loaded
...
USER_TOOLS_DIR = Path("~/.friday/tools").expanduser()
...
def _load_user_tool(path: Path, registry: ToolRegistry) -> None:
    spec = importlib.util.spec_from_file_location(f"friday_user_tools.{path.stem}", path)
...
def load_user_tools(registry: ToolRegistry) -> int:
    """Import every user-authored tool from ~/.friday/tools (called at startup)."""
