...
def test_read_document_markitdown_text(tmp_path):
    from friday.tools.documents import register
    reg = ToolRegistry()
    register(reg)
    f = tmp_path / "note.md"
    f.write_text("# Title\n\nHello world.")
...
def test_about_friday_tool():
    from friday.tools.selfdoc import register
    reg = ToolRegistry()
    register(reg)
    out = reg.execute("about_friday", {"topic": "provider"})
...
def test_create_tool_authoring(tmp_path, monkeypatch):
    import friday.tools.authoring as authoring
...
def test_gws_without_cli(monkeypatch):
    import friday.tools.gws as gws
