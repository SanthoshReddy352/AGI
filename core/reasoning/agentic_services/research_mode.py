"""ResearchWorkflow — agentic service for LLM-driven research loops.

Track 5.2e: this is NOT a linear slot-fill workflow. It's an LLM
agentic loop — search → LLM summarize → optional file save — driven
by model output, not by a fixed sequence of ask-steps. Templating it
would mean encoding "ask the LLM to decide what to do next" in YAML,
which the template model intentionally does not support. It lives
under `core/reasoning/agentic_services/` (renamed from
`core/reasoning/workflows/`).

Phase 9: registered with WorkflowOrchestrator. Handles "research X"
intents by chaining: web/world_monitor search → LLM summarization →
optional file save.

The class name is kept as `ResearchWorkflow` (and `name = "research_mode"`)
