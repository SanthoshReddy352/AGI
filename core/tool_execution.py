...
class OrderedToolExecutor:
    ...
    def execute(self, plan: ToolPlan, user_text: str, turn=None):
        ...
        if plan.mode == "planner":
            ...
            # Track 3.2b: was `self.app.router.process_text(user_text)`,
            # which re-entered the v1 dispatch and built another ToolPlan
            # only to land in chat anyway. The v2 equivalent is a direct
            # `llm_chat` execution — same response, no re-routing, one
            # less router fire to count.
            executor = getattr(self.app, "capability_executor", None)
            if executor is not None and self.app.capability_registry.has_capability("llm_chat"):
                result = executor.execute("llm_chat", user_text, {"query": user_text})
                return result.output if result.ok else (result.error or "")
            # Last-resort fallback for partial test apps that didn't wire
            # `capability_executor` — keep the legacy entry so existing
