...
class LocalModelManager:
    ...
    def refresh_from_config(self, config=None):
        ...
        self._profiles = {
            "chat": ModelProfile(
                role="chat",
                path=self._resolve_path(self._config_get("models.chat.path", os.path.join(models_dir, "Qwen3.5-0.8B-Q4_K_M.gguf"))),
            ...
            "tool": ModelProfile(
                role="tool",
                path=self._resolve_path(self._config_get("models.tool.path", os.path.join(models_dir, "Qwen3.5-4B-Q4_K_M.gguf"))),
    ...
    def get_chat_model_if_loaded(self):
        return self.get_model_if_loaded("chat")

    def get_tool_model_if_loaded(self):
        return self.get_model_if_loaded("tool")

    def get_model(self, role, block=True):
        ...
        if not block:
            if self._locks[role].acquire(blocking=False):
        ...
        with self._locks[role]:
            ...
            try:
                ...
                exc_str = str(exc)
                if "0xc000001d" in exc_str or "WinError -1073741795" in exc_str or "Illegal instruction" in exc_str.lower():
                    logger.error(
                        "Error initializing %s model: %s\n"
                        "  [Windows] llama-cpp-python requires AVX2 CPU instructions not available on this CPU.\n"
                        "  Fix: rebuild without AVX2:\n"
                        "    pip uninstall llama-cpp-python -y\n"
                        "    $env:CMAKE_ARGS='-DLLAMA_AVX2=OFF -DLLAMA_AVX512=OFF'; pip install llama-cpp-python --no-binary :all:",
                        role, exc,
                    )
                else:
    ...
    def get_model_if_loaded(self, role: str):
        """Return the model only if it is already loaded. Never blocks
        on the load lock. Returns None when the model is still loading
        or failed — lets the caller fall back to a non-LLM path instead
        of blocking for seconds on disk I/O."""
        if role in self._models:
            return self._models[role]
        if role in self._load_failed:
            return None
