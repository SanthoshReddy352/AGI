...
def strip_model_artifacts(text: Any) -> str:
    ...
    # Remove /no_think and /think anywhere — including inline and inside quotes.
    cleaned = re.sub(r"\s*/no_think\b", "", cleaned)
    cleaned = re.sub(r"\s*/think\b", "", cleaned)
...
def _strip_guard_leak(text: str, assistant_name: str) -> str:
    """Remove a parroted identity-guard sentence the small model sometimes
    emits verbatim from its own system prompt.

    The 0.8B chat model occasionally opens a reply with the meta-disclaimer
    it was instructed with — e.g. "Understood. I am Friday, the assistant,
    not the user." (the 2026-05-29 bug). That sentence is scaffolding, never
    meant for the user, so we strip it regardless of whose name is involved.
    """
    guard_leak = re.compile(
        r"\b(?:understood[.,!]?\s+)?i\s*(?:'m|am)\s+"
        r"(?:" + re.escape(assistant_name) + r"\s*,?\s+)?"
        r"the\s+assistant\s*,?\s+not\s+the\s+user\s*[.!]?\s*",
        re.IGNORECASE,
    )
    return guard_leak.sub("", text).lstrip()


def strip_user_impersonation(text: Any, user_name: str, assistant_name: str = "FRIDAY") -> str:
    """Repair the assistant leaking its identity guard or claiming the *user's*
    name as its own.

    The 0.8B chat model intermittently (a) parrots its own guard sentence
    ("I am Friday, the assistant, not the user.") and (b) drifts into the
    user's identity — e.g. "I'm Friday, an assistant named Luffy." when the
    user's profile name is Luffy. The prompt guards reduce both but don't
    eliminate them, so this is the deterministic safety net applied to both
    the spoken and returned text.

    The parroted-guard scrub runs first and is name-independent. The
    self-identification repair then rewrites clear phrasings ("I am <name>",
    "my name is <name>", "this is <name>", "assistant named/called <name>")
    so legitimate uses — addressing the user by name ("Luffy, here's what I
    found") — are left untouched. The user's name is replaced with the
    assistant's so the sentence stays grammatical.
    """
    if not isinstance(text, str) or not text:
        return text if isinstance(text, str) else ""
    text = _strip_guard_leak(text, assistant_name)
    name = (user_name or "").strip()
    if not name:
        return text
    escaped = re.escape(name)
    self_intro = re.compile(
        r"\b(i\s+am|i'm|my\s+name\s+is|this\s+is)\s+" + escaped + r"\b",
        re.IGNORECASE,
    )
    named = re.compile(
        r"\b(assistant\s+(?:named|called)|named|called)\s+" + escaped + r"\b",
        re.IGNORECASE,
    )
    text = self_intro.sub(lambda m: f"{m.group(1)} {assistant_name}", text)
    text = named.sub(lambda m: f"{m.group(1)} {assistant_name}", text)
    return text
