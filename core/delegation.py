...
class WorkflowAgent:
    ...
    def handle(self, request: DelegationRequest) -> DelegationResult:
        ...
        # Track 3.2b: was `self.app.router.process_text(request.task)` —
        # v1 entry. `dispatch_sub_text` is the v2-native equivalent:
        # intent classification → capability execution without entering
        # a new TurnManager turn (this PlannerAgent is already inside
        # an active turn).
        response = self.app.dispatch_sub_text(request.task)
        return DelegationResult(
            summary=response,
...
class ResearchAgent:
    ...
    def handle(self, request: DelegationRequest) -> DelegationResult:
        # Track 3.2b: see PlannerAgent above — `dispatch_sub_text` is the
        # v2-native equivalent of `router.process_text` for sub-dispatches.
        response = self.app.dispatch_sub_text(request.task)
        return DelegationResult(
            summary=response,
