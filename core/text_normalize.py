...
def _load_yaml_substitutions() -> dict[str, str]:
    """Load phonetic corrections from config/stt_substitutions.yaml.

    Returns an empty dict if the file is missing or unparseable so the
    hard-coded table always works even without PyYAML installed.
    """
    config_path = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "config", "stt_substitutions.yaml"
    )
    ...
    try:
        ...
        return {str(k).lower(): str(v) for k, v in subs.items() if k and v}
    except Exception:
        return {}


_STT_TYPOS.update(_load_yaml_substitutions())
