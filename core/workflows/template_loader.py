...
class WorkflowTemplateStep:
    ...
    # Track 5.2a — multi-turn slot-fill primitive. An ask-step has no
    # capability; it parks the workflow with a question and waits for
    # the user's next turn. `extract_with` is optional — when set it
    # names a capability that parses the answer; otherwise the raw user
    # text fills the slot.
    ask: str = ""
    slot: str = ""
    extract_with: str = ""
...
class WorkflowTemplate:
    ...
    # Track 5.2c — mid-flow cancel predicate. Names a capability that, when
    # truthy, signals the orchestrator to drop this workflow (e.g. the user
    # named a different filename mid-flow). Evaluated on every resume turn
    # before slot-fill advances.
    cancel_when: str = ""
    cancel_response: str = ""
...
class WorkflowTemplateLoader:
    ...
    def load_file(self, path: str) -> WorkflowTemplate:
        ...
        tpl = WorkflowTemplate(
            ...
            source_path=path,
            cancel_when=str(data.get("cancel_when") or "").strip(),
            cancel_response=str(data.get("cancel_response") or "").strip(),
        )
    ...
    def _build_step(self, path: str, idx: int, raw: Any) -> WorkflowTemplateStep:
        ...
        if "step_id" not in raw:
            raise TemplateError(f"{path}: step #{idx} missing required 'step_id'")
        # Track 5.2a: a step is either a capability-step (has `capability`)
        # OR an ask-step (has `ask` + `slot`, no `capability`). Validate the
        # exclusive choice.
        has_capability = "capability" in raw and str(raw["capability"]).strip()
        has_ask = "ask" in raw and "slot" in raw
        if not has_capability and not has_ask:
            raise TemplateError(
                f"{path}: step #{idx} must declare either 'capability' "
                f"or 'ask' + 'slot' (multi-turn slot-fill)"
            )
        if has_capability and has_ask:
            raise TemplateError(
                f"{path}: step #{idx} cannot declare both 'capability' "
                f"and 'ask' — pick one"
            )
        return WorkflowTemplateStep(
            ...
            extract_with=str(raw.get("extract_with") or "").strip(),
        )
