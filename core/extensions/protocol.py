...
class ExtensionContext:
    ...
    def register_capability(
        ...
        # NOTE: the router's keyword is ``capability_meta`` (see
        # core/router.py:register_tool), NOT ``metadata``. Passing the wrong
        # kwarg used to raise TypeError that the bare except below swallowed,
        # so every Extension capability that carried metadata (all of
        # workspace_agent's email/calendar/drive tools) silently never landed
        # in ``router._tools_by_name`` — IntentRecognizer couldn't see them and
        # the turn fell through to the chat model, which fabricated replies.
        ...
        if router is not None and hasattr(router, "register_tool"):
            try:
                router.register_tool(spec, handler, capability_meta=metadata)
            except Exception:
                logger.exception(
                    "[extensions] router.register_tool failed for %s",
                    spec.get("name", "?"),
                )
