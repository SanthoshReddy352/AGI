...
Track 2.2c status (2026-05-18): `EntityExtractor` is now ACTIVATED.
`MemoryBroker.curate` calls `process_turn` to populate the entity/fact
graph AND `extract_user_facts` to route first-party user facts through
`MemoryFacade.remember`. So every completed turn populates the typed
graph (third-party people, projects, tools, places) AND the canonical
facade (the user's own name, location, role). The two paths share the
facade's normalization layer for first-party facts — no spelling drift.
...
class EntityExtractor:
    ...
    Track 2.2c (Consolidation Direction): activated in `MemoryBroker.curate`
    so every completed turn populates the entity/fact/relationship tables.
    First-party user facts (the user's own name, location, …) also flow
    through the canonical MemoryFacade via `extract_user_facts()` so the
    same value lands in both the typed graph AND the facade — no spelling
    ...
    def extract_user_facts(self, text: str) -> dict[str, str]:
        """Return `{key: value}` pairs for FIRST-PARTY user facts in `text`
        (the user describing themselves, not a third party).

        Used by `MemoryBroker.curate` to route through `MemoryFacade.remember`
        in addition to the typed-graph upsert. The patterns are narrower than
        `_PERSON_PATTERNS` above on purpose: only utterances where the user
        is the SUBJECT count as user facts (`my name is X`, `call me X`),
        whereas `my friend X` / `X said` are third-party references that
        belong on the graph but not in the user's profile facts.
        """
        if not isinstance(text, str) or not text:
            return {}
        facts: dict[str, str] = {}

        m = re.search(r"\bmy\s+name\s+is\s+([A-Z][a-zA-Z\-]{1,30})\b", text)
        if m:
            facts["name"] = m.group(1)
        m = re.search(r"\bcall\s+me\s+([A-Z][a-zA-Z\s\-]{1,30})", text, re.IGNORECASE)
        if m and "name" not in facts:
            facts["name"] = m.group(1).strip().split(" ")[0]

        m = re.search(
            r"\b(?:i\s+live\s+in|i'?m\s+based\s+in|i\s+am\s+from|i'?m\s+from|my\s+location\s+is)\s+([A-Z][a-zA-Z\-]{1,40})\b",
            text,
        )
        if m:
            facts["location"] = m.group(1)

        m = re.search(
            r"\bi\s+work\s+as\s+(?:an?\s+)?([a-zA-Z][a-zA-Z\s\-]{1,40})(?=\s+(?:and|but|because|so)\b|[,.!?]|$)",
            text,
            re.IGNORECASE,
        )
        if m:
