...
from core.tool_result import classify as _classify_result
...
class CommandRouter:
    def __init__(self, event_bus):
        ...
        # Adaptive Intent Phase 3: cheap fuzzy layer between regex and
        # embeddings — catches STT/typo near-misses before paying for cosine.
        self.lexical_router = None
        if os.getenv("FRIDAY_DISABLE_LEXICAL_ROUTER") != "1":
            try:
                from core.lexical_router import LexicalRouter  # noqa: PLC0415
                self.lexical_router = LexicalRouter()
            except Exception as exc:
                logger.warning("[router] Lexical router unavailable: %s", exc)
    ...
    def _invoke_route(self, route, text, args):
        tool_name = route["spec"]["name"]
        try:
            response = route["callback"](text, args)
        except Exception as e:
            logger.error("Error executing tool '%s': %s", tool_name, e)
            return f"Error running command: {e}"
        outcome = _classify_result(tool_name, response)
        if outcome == "retryable":
            logger.warning("[router] %s returned retryable result; retrying once.", tool_name)
            try:
                response = route["callback"](text, args)
                outcome = _classify_result(tool_name, response)
            except Exception as e:
                logger.error("[router] Retry failed for '%s': %s", tool_name, e)
        if outcome == "fatal":
            logger.error("[router] Fatal result from '%s': %.200s", tool_name, str(response))
            audit = getattr(self, "audit_trail", None)
            if audit:
                try:
                    audit.log(tool_name, ok=False, summary=str(response)[:200])
                except Exception:
                    pass
        elif outcome == "soft_fail":
            logger.warning("[router] soft_fail from '%s'", tool_name)
        self._remember_tool_use(tool_name, args)
        return self._finalize_response(response)
    ...
    def _find_best_route(self, text, min_score=20):
        # Track 4.1b: delegate to the canonical RouteScorer when the app
        # wired one (production). The local scoring loop is kept as a
        # fallback for partial test apps that construct CommandRouter
        # without a scorer. Once every external caller migrates to
        # `app.route_scorer.find_best_route(...)`, this method and its
        # `_score_route` helper delete entirely.
        scorer = getattr(self, "route_scorer", None)
    ...
    def _default_aliases_for(self, tool_name):
        # Track 4.1b deeper: defaults consolidated into
        # `core/reasoning/routing_defaults.py` as the single source of truth.
        from core.reasoning.routing_defaults import DEFAULT_ALIASES  # noqa: PLC0415
        return DEFAULT_ALIASES.get(tool_name, set())

    def _default_context_terms_for(self, tool_name):
        from core.reasoning.routing_defaults import DEFAULT_CONTEXT_TERMS  # noqa: PLC0415
        return DEFAULT_CONTEXT_TERMS.get(tool_name, set())

    def _default_patterns_for(self, tool_name):
        from core.reasoning.routing_defaults import DEFAULT_PATTERNS  # noqa: PLC0415
        return DEFAULT_PATTERNS.get(tool_name, [])
