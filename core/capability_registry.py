...
class CapabilityRegistry:
    ...
    def register(self, tool_spec: dict, handler: Handler, metadata: dict | None = None) -> "CapabilityDescriptor":
        """Track 4.1 (Consolidation Direction): canonical short-name entry
        point. The Direction calls for `app.capabilities.register(spec, handler)`
        as the only registration API; this method is that public name.

        Plugins that still call `app.router.register_tool(...)` continue
        to work — the router internally forwards to this registry, so
        both paths converge on the same store. Migrating to
        `app.capabilities.register(...)` is the recommended path; the
        legacy router entry will get a deprecation log when the full
        plugin migration starts (gated on the pattern-matching logic
        moving out of CommandRouter).
        """
        return self.register_tool(tool_spec, handler, metadata=metadata)
...
class CapabilityExecutor:
    def __init__(self, registry: CapabilityRegistry):
        ...
        # Track 6.3 (2026-05-23): screen lock injected post-construction.
        # When set and `lock.is_locked()` is True, screen-dependent
        # capabilities in ScreenLock.BLOCKED_WHEN_LOCKED short-circuit with a
        # refusal; everything else runs.
    ...
    def execute(self, name: str, raw_text: str, args: dict | None = None) -> CapabilityExecutionResult:
        ...
        # Lock gate: while the OS screen is locked, screen-dependent tools
        # (browser automation, app/file launching, screenshots, vision, …)
        # are refused; everything else still works.
        lock = getattr(self, "screen_lock", None)
        if lock is not None and not lock.is_allowed(name):
            return CapabilityExecutionResult(
                ok=True,  # not an error — a refusal the user can act on
                name=name,
                output=(
                    f"The screen is locked, so I can't run '{name}' right "
                    "now — it needs the desktop. Unlock the screen first."
                ),
