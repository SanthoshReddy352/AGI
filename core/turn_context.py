...
class TurnContext:
    ...
    # Phase 5: per-step structured observations stashed by capability handlers
    # (e.g. nmap_parser.py output). Keys are step node_ids. Downstream steps
    # and the replanner read this for ${step_id.field} references and
    # observation-based replanning decisions.
    observations: dict[str, dict] = field(default_factory=dict)
    # Track 1.3b: per-turn references registry. Plugins emitting numbered
    # lists call `references.register(items, kind=...)`. Set by TurnManager
    # at turn start; remains None when TurnContext is constructed in tests
    # without an app, so accessors must guard.
    references: Any = None
