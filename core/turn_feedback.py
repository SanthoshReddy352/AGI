...
class RuntimeMetrics:
    def __init__(self, max_records: int = 20):
        ...
        # Track 3.1 (Consolidation Direction): per-turn router-fire counter.
        # The v2 dispatch contract is exactly one routing decision per turn
        # (intent → resolver → execute). Anything that increments this past
        # 1 is a double-route bug. Reset at turn start; the
        # `test_single_routing_decision_per_turn` pin asserts == 1.
        self.router_fires_last_turn: int = 0

    def begin_turn(self) -> None:
        """Reset per-turn counters before a new turn dispatches."""
        with self._lock:
            self.router_fires_last_turn = 0

    def increment_router_fire(self) -> None:
        """Called once per routing decision made for the current turn."""
        with self._lock:
            self.router_fires_last_turn += 1
