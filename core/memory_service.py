...
class MemoryService:
    ...
    Track 2.3 (Consolidation Direction): the optional `mem0_client` and
    `extractor` parameters were removed. Mem0 is deleted; the canonical
    writer/reader is now `MemoryBroker.facts` (MemoryFacade). The kwargs
    are still accepted-and-ignored for one release so any out-of-tree
    callers don't crash; they will be removed in a follow-up.
    """

    def __init__(self, context_store, memory_broker=None, mem0_client=None, extractor=None):
        if mem0_client is not None or extractor is not None:
            logger.debug(
                "[memory_service] mem0_client / extractor kwargs are ignored "
                "after Track 2.3 (Mem0 was deleted; MemoryFacade is canonical)."
            )
    ...
    def build_context_bundle(self, session_id: str, query: str) -> dict:
        ...
        # Track 2.3: Mem0 search block deleted. `user_facts` is now
        # populated by `MemoryBroker.build_context_bundle` via the
        # canonical `MemoryFacade.render_user_facts` call.
    ...
    def record_turn(
        ...
        # Track 2.3: Mem0 extraction queue removed. Ambient fact extraction
        # now runs synchronously through `MemoryBroker.curate` which writes
        # via the canonical MemoryFacade — see Track 2.2b.
    ...
    def set_pending_intent(self, session_id: str, payload: dict) -> None:
        self._store.set_pending_intent(session_id, payload)

    def clear_pending_intent(self, session_id: str) -> None:
        self._store.clear_pending_intent(session_id)
