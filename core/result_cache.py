...
TTL policy (revised 2026-05-25 — fresh-compute by default):
  - latency_class == "background"  (heavy / high-compute, e.g. research)
    AND side_effect_level == "read"                          → 900 s (cache)
  - everything else (interactive/slow tools: weather, news,
    email, web links, quick answers, etc.)                   → 0 s (no cache)
  - anything with side_effect_level in {"write", "critical"} → 0 s (no cache)

Rationale: the user wants ordinary prompts recomputed every time so results
are never stale; only expensive background research is worth caching.
"""
...
_HEAVY_TTL = 900  # background / high-compute read results (e.g. research)
...
def _ttl_for(descriptor) -> int:
    # Default (no descriptor) → don't cache; recompute every time.
    if descriptor is None:
        return 0
    side_effect = getattr(descriptor, "side_effect_level", "read") or "read"
    if side_effect in ("write", "critical"):
        return 0
    latency_class = getattr(descriptor, "latency_class", "interactive") or "interactive"
    # Only heavy/high-compute background reads are worth caching.
    return _HEAVY_TTL if latency_class == "background" else 0
