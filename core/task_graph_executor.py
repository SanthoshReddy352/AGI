...
class TaskGraphExecutor:
    ...
    def _run_node(
        ...
        # Phase 5: resolve ${step_id.field.path} placeholders against the
        # turn's structured observations. Falls through silently when the
        # observation is missing (placeholder left in-place; downstream
        # handler can surface the absence in its response).
        args = self._resolve_observation_refs(args)
        # Pass the step's node_id through to the handler so it can stash
        # observations under the right key (the executor does NOT know
        # the handler-internal observation contract beyond this hand-off).
        if step.node_id and "__step_id__" not in args:
            args["__step_id__"] = step.node_id
    ...
    # ------------------------------------------------------------------
    # Phase 5: ${step_id.field.path} substitution from observations
    # ------------------------------------------------------------------

    _OBS_REF_RE = re.compile(r"\$\{\s*([A-Za-z_][A-Za-z0-9_]*)\.([A-Za-z0-9_.]+?)\s*\}")

    def _resolve_observation_refs(self, args: dict) -> dict:
        """Replace ``${step_id.dotted.path}`` placeholders in *args* (string
        values only) with values pulled from the active turn's observations.

        Missing observations / missing paths leave the placeholder unchanged.
        """
        try:
        ...
        for key, value in args.items():
            out[key] = self._resolve_single_value(value, store)
        return out

    def _resolve_single_value(self, value, store: dict):
        if isinstance(value, str):
            def replace(m):
                step_id = m.group(1)
                path = m.group(2)
                obs = store.get(step_id)
                if obs is None:
                    return m.group(0)
                resolved = self._lookup_path(obs, path)
                if resolved is None:
                    return m.group(0)
                if isinstance(resolved, (list, tuple)):
                    return ",".join(str(x) for x in resolved)
                return str(resolved)
            return self._OBS_REF_RE.sub(replace, value)
        if isinstance(value, list):
            return [self._resolve_single_value(v, store) for v in value]
        if isinstance(value, dict):
            return {k: self._resolve_single_value(v, store) for k, v in value.items()}
        return value

    @staticmethod
    def _lookup_path(obs: dict, dotted: str):
        """Walk a dotted path inside an observation dict.

        Supports the two common shortcuts:
        - top-level keys (`status`, `summary`)
        - nested `structured_data.foo.bar`
        Also auto-prepends `structured_data` if the first segment isn't a
        top-level key but exists under structured_data.
        """
        parts = dotted.split(".")
        candidates = [obs]
        if "structured_data" in obs and parts and parts[0] not in obs:
            candidates.append({"structured_data": obs["structured_data"]})
            candidates[-1].update(obs["structured_data"])  # type: ignore[union-attr]
