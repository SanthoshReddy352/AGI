...
class ConversationAgent:
    """Track 3.2 status: the v1 turn-dispatch entry points
    (`build_tool_plan`, `execute_tool_plan`, `curate_memory`,
    `process_turn`) are NO LONGER called from production — `TurnManager`
    routes every turn through the v2 `TurnOrchestrator`. These methods
    are retained as dead code so existing direct callers (a few tests
    and the `plan_turn` API used by `test_hybrid_architecture`) keep
    working; they will be deleted when those callers migrate.
    """
    ...
    def plan_turn(self, text: str, source: str = "user", context_bundle: dict | None = None, style_hint: str = ""):
        # Track 3.2c: build_plan deleted from CapabilityBroker; route
        # through the v2 PlannerEngine instead. Build a minimal ctx shape
        # that carries the kwargs the engine reads. Fall back to a
        # locally-constructed PlannerEngine when the app doesn't have
        # one wired (test apps using SimpleNamespace).
        from types import SimpleNamespace  # noqa: PLC0415
        from core.planning.planner_engine import PlannerEngine  # noqa: PLC0415
        ctx = SimpleNamespace(
            turn_id="",
            source=source,
        ...
        engine = getattr(self.app, "planner_engine", None) or PlannerEngine(
            self.app.capability_broker
        )
        tool_plan = engine.plan(text, ctx=ctx)
