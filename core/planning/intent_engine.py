...
class IntentEngine:
    ...
    def classify(self, text: str, ctx=None) -> IntentResult:
        ...
        if actions:
            first = actions[0]
            # Calibrated confidence: a parser may tag an action with its own
            # `confidence` when the match is genuinely ambiguous (a broad anchor
            # rather than an exact verb+object). Default 1.0 keeps every existing
            # exact-regex match on the high-confidence fast path — zero behaviour
            # change unless a parser opts in. A value below HIGH_THRESHOLD lets
            # the orchestrator fall through to the planner / confirmation band
            # instead of dispatching silently.
            confidence = self._action_confidence(first)
            return IntentResult(
                tool=first.get("tool"),
    ...
    @staticmethod
    def _action_confidence(action: dict) -> float:
        """Confidence for a regex action. Honors an explicit `confidence` on the
        action (clamped to [0, 1]); otherwise an exact regex match is 1.0."""
        raw = action.get("confidence", 1.0)
        try:
