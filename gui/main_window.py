...
# Minimal markdown → HTML pass for chat bubbles. We HTML-escape the text
# first (so `<script>` stays text), then re-introduce a handful of inline
# formats: **bold**, *italic*, `code`. Heavier markdown (lists, headers,
# links) is intentionally left to the LLM — the chat bubble is for
# conversational output, not rendered documents.
...
class MainWindow(QMainWindow):
    ...
    def handle_return_pressed(self):
        ...
        if text.startswith("file://"):
            ...
            try:
                ...
                pass
        # Run OFF the Qt main thread: a slash/`!`-shell command that hits a
        # network capability would otherwise block the event loop and trip
        # the desktop "Force Quit" dialog. Responses arrive via the event bus.
        import threading
        threading.Thread(
            target=self.app_core.process_input,
            args=(text,),
            kwargs={"source": "gui"},
            daemon=True,
        ).start()
    ...
    def _insert_bubble(self, role: str, text: str):
        ...
        # Markdown inline render so **bold** / *italic* / `code` reach
        # the user as real formatting rather than literal asterisks.
        safe = _markdown_inline_to_html(display_text).replace("\n", "<br/>")
