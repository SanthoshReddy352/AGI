...
def _clamp_detail(text: str, max_chars: int = 60, max_token: int = 22) -> str:
    """Keep a weather-detail string narrow so it can't stretch the HUD column.

    A word-wrapped QLabel sizes its minimum width to the widest unbreakable
    token, so any long run with no spaces (a URL, a stack-y error) forces the
    left column wide. We hard-break overlong tokens and cap total length.
    """
    text = " ".join((text or "").split())  # collapse whitespace/newlines
    broken = []
    for token in text.split(" "):
        while len(token) > max_token:
            broken.append(token[:max_token])
...
def format_weather_status(weather):
    ...
    if status != "success":
        return {
            ...
            "details": _clamp_detail(str(weather.get("message") or "Waiting for update")),
        }
...
class WeatherFetchThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.weather_ready.emit({"status": "error", "message": _weather_error_message(exc)})


def _weather_error_message(exc: Exception) -> str:
    """A short, human message for the weather panel.

    The raw exception string for a network failure is a single very long
    unbreakable token (e.g. ``HTTPSConnectionPool(host='api.open-meteo.com'
    …url: /v1/forecast?latitude=…)``). A word-wrapped QLabel cannot break
    inside a word, so its minimum width balloons to fit that token and
    stretches the whole left column, pushing the center/right columns
    off-screen. Returning a short phrase with normal spaces keeps the label
    narrow and the 3:6:3 column layout intact.
    """
...
def _md_inline_to_html(s: str) -> str:
    """Inline markdown → Qt rich-text HTML (bold/italic/code/links)."""
    ...
    s = re.sub(r"(?<![\*\w])\*([^\*\n]+?)\*(?!\w)", r"<i>\1</i>", s)
    s = re.sub(r"(?<![_\w])_([^_\n]+?)_(?!\w)", r"<i>\1</i>", s)
    # [text](url) links, then bare URLs
    s = re.sub(r"\[([^\]]+)\]\((https?://[^)\s]+)\)", r'<a href="\2">\1</a>', s)
    s = re.sub(r"(?<!\"|>)(https?://[^\s<]+)", r'<a href="\1">\1</a>', s)
    return s


def _markdown_to_qt_html(text: str) -> str:
    """Render assistant markdown the way Telegram does — bold, italics,
    inline code, fenced code blocks, bullet/numbered lists, headings, links —
    into the HTML subset QLabel rich text understands. Defensive: any failure
    falls back to escaped plain text so a bubble never renders raw tags.
    """
    import html as _html
    try:
        ...
        for line in lines:
            if line.strip().startswith("```"):
                if not in_code:
                    close_list(); in_code = True; code_buf = []
                else:
                    in_code = False
                    body = _html.escape("\n".join(code_buf))
                    out.append(
            ...
            stripped = line.strip()
            if not stripped:
                close_list(); out.append("<br>"); continue
            m = re.match(r"^(#{1,6})\s+(.*)$", stripped)
            if m:
                close_list()
                out.append("<b>" + _md_inline_to_html(m.group(2)) + "</b><br>")
                continue
            m = re.match(r"^[-*•]\s+(.*)$", stripped)
            if m:
                if list_mode != "ul":
                    close_list(); out.append("<ul>"); list_mode = "ul"
                out.append("<li>" + _md_inline_to_html(m.group(1)) + "</li>")
                continue
            m = re.match(r"^(\d+)[.)]\s+(.*)$", stripped)
            if m:
                if list_mode != "ol":
                    close_list(); out.append("<ol>"); list_mode = "ol"
                out.append("<li>" + _md_inline_to_html(m.group(2)) + "</li>")
                continue
            close_list()
            out.append(_md_inline_to_html(line) + "<br>")
        close_list()
        if in_code:  # unterminated fence
            out.append("<pre style='white-space:pre-wrap;'>"
                       + _html.escape("\n".join(code_buf)) + "</pre>")
        html = "".join(out)
        while html.endswith("<br>"):
            html = html[:-4]
        return html or _html.escape(str(text))
    except Exception:
...
class _TypewriterEffect:
    ...
    def __init__(self, label: "QLabel", full_text: str, parent: "QWidget",
        ...
        self._pos = 0
        # render_fn(full_text) renders the finished message (e.g. as markdown
        # HTML). During the animation we show escaped plain text + a cursor;
        # the rich render is applied only once, at the end.
    ...
    def _tick(self) -> None:
        ...
        if self._pos >= len(self._full):
            ...
            self._finalize()
            return
        self._label.setText(self._full[:self._pos] + "▋")

    def _finalize(self) -> None:
        if self._render_fn is not None:
            self._render_fn(self._full)
        else:
...
class ChatBubble(QFrame):
    ...
    def __init__(self, message: _ChatMessage, theme: Theme, parent=None):
        ...
        # Assistant + system messages render markdown (bold, lists, code,
        # links) like the Telegram bridge does; user text stays literal so
        # anything they typed isn't reinterpreted as formatting.
        self._rich = message.role in ("assistant", "system")
        if self._rich:
        ...
        self.render_text(message.text)
        self._stream_text = message.text
    ...
    def render_text(self, text: str) -> None:
        """Set the bubble's text, rendering markdown→HTML for rich roles."""
        if self._rich:
            self.text_label.setTextFormat(Qt.TextFormat.RichText)
            self.text_label.setText(_markdown_to_qt_html(text))
        else:
            self.text_label.setText(text)

    def start_typewriter(self, text: str) -> None:
        """Animate assistant reply text appearing progressively, then render
        the finished message as markdown HTML."""
        self._stream_text = text
        # Animate as plain text (so partial markdown/HTML never renders
        # broken); render_text() switches back to rich HTML at the end.
        if self._rich:
            self.text_label.setTextFormat(Qt.TextFormat.PlainText)
        self._typewriter = _TypewriterEffect(
            self.text_label, text, self, render_fn=self.render_text
        )
    ...
    def set_streaming_text(self, text: str) -> None:
        ...
        Streams as plain text — partial markdown ('**bo') would render as
        broken HTML in a rich-text label — then `finalize_render()` swaps in
        the formatted version once the full message has arrived.
        """
        if self._typewriter:
            self._typewriter.finish()
            self._typewriter = None
    ...
    def finalize_render(self) -> None:
        """Re-render the fully-streamed text as markdown HTML."""
        if self._rich:
            self.text_label.setTextFormat(Qt.TextFormat.RichText)
        self.render_text(self._stream_text)
...
class ChatView(QScrollArea):
    ...
    def finalize_streaming_bubble(self, model_lane=None, model_label=None) -> None:
        ...
        if self._streaming_bubble:
            ...
            self._streaming_bubble.finalize_render()  # plain stream → markdown HTML
...
class JarvisHUD(QMainWindow):
    ...
    def __init__(self, app_core):
        ...
        # ── VOICE panel (top) — all voice controls in one place ──
        ...
        self.voice_mode_combo.setFixedHeight(38)
        self.voice_mode_combo.setSizePolicy(
        ...
        # Input device selector (inline — no separate frame)
        self.mic_label = QLabel("INPUT DEVICE")
        self.voice_panel_frame.body.addWidget(self.mic_label)
        self.mic_combo = QComboBox()
        self.mic_combo.setFixedHeight(38)
        self.mic_combo.setSizePolicy(
            QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed
        )
        self.mic_combo.currentIndexChanged.connect(self._on_mic_combo_changed)
        self.voice_panel_frame.body.addWidget(self.mic_combo)
        ...
        self.voice_panel_frame.body.addStretch(1)
        right.addWidget(self.voice_panel_frame, stretch=3)

        # ── SYSTEM PULSE (middle) — more height for animated bars ──
        ...
        right.addWidget(self.telemetry_panel, stretch=3)
        ...
        right.addWidget(self.models_panel_frame, stretch=2)

        # Minimum heights so content never overlaps when column is short
        self.voice_panel_frame.setMinimumHeight(340)
        self.telemetry_panel.setMinimumHeight(160)
        ...
        # Microphone device discovery (10s refresh)
        self._mic_discovery_thread = None
        self._mic_timer = QTimer(self)
        self._mic_timer.timeout.connect(self._start_mic_discovery)
        self._mic_timer.start(10000)
        QTimer.singleShot(250, self._start_mic_discovery)
    ...
    def _apply_theme(self, theme: Theme) -> None:
        ...
        self.mic_label.setStyleSheet(label_style(theme, theme.accent, 11, "bold"))
        self.mic_combo.setStyleSheet(combo_style(theme))
        for label in self._voice_labels:
    ...
    def _dispatch_input(self, text: str):
        """Run process_input OFF the Qt main thread.

        Normal turns already hand off to the app's task_runner, but the
        slash/`!`-shell prefix short-circuit runs inline — and a slash
        command that hits a network capability (e.g. `/web` → SearchFlox's
        blocking urllib call) would freeze the event loop and trip the
        desktop's "Force Quit" dialog (observed 2026-05-25: `/web …` →
        `zsh: killed`). Dispatching on a worker QThread keeps the UI
        responsive; responses still arrive via the event bus.
        """
        worker = _InputWorker(self.app_core, text)
        if not hasattr(self, "_input_workers"):
            self._input_workers = []
        self._input_workers.append(worker)

        def _cleanup():
            try:
                self._input_workers.remove(worker)
            except ValueError:
                pass
    ...
    def _on_mic_combo_changed(self, index):
        if index < 0:
            return
        text = self.mic_combo.itemText(index)
        if text in {"Scanning...", "No microphone found"}:
            return
        self.on_mic_selected(self.mic_combo.itemData(index))
    ...
    def _start_mic_discovery(self):
        if self._mic_discovery_thread and self._mic_discovery_thread.isRunning():
            return
        if self.mic_combo.count() == 0:
            self.mic_combo.addItem("Scanning...", None)
        self._mic_discovery_thread = DeviceDiscoveryThread()
        self._mic_discovery_thread.devices_found.connect(self._on_mic_devices_found)
        self._mic_discovery_thread.start()

    def _on_mic_devices_found(self, devices):
        current_id = self.mic_combo.currentData()
        ...
        if not devices:
            ...
            for device in devices:
                ...
                self.mic_combo.addItem(f"{prefix}{device.label}{suffix}", device.target)
        if current_id is not None:
            for index in range(self.mic_combo.count()):
                if self.mic_combo.itemData(index) == current_id:
    ...
    def closeEvent(self, event):
        # 2026-05-24 — fast, bounded exit. `app_core.shutdown()` joins
        # its inner threads with a wall-clock deadline (default ~2.5s),
        # so closeEvent itself returns quickly. After that we schedule a
        # hard `os._exit(0)` via QTimer so even a stuck daemon thread or
        # a pending Playwright handshake can't keep the Qt "force quit"
        # dialog open. The graceful shutdown still runs first — this is
        # only a backstop, not the primary path.
        try:
        ...
        try:
            ...
            try:
                from PyQt5.QtCore import QTimer as _QT5  # noqa: PLC0415
                _QT5.singleShot(800, lambda: os._exit(0))
            except Exception:
                # No Qt available (unit-test path) — just exit straight.
                os._exit(0)
...
def start_hud(app_core, *, greeting=None):
    ...
    # Fire the startup greeting on the next event-loop tick so all HUD
    # event-bus subscribers are already wired — no sleep needed.
    if greeting:
        QTimer.singleShot(0, lambda: app_core.event_bus.publish("voice_response", greeting))
    sys.exit(app.exec())
