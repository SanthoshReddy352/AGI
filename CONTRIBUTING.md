...
Thanks for your interest in FRIDAY. This is a cloud-only assistant; the whole app
lives in the [`friday/`](friday/) package.
...
python -m pytest friday/tests/ -q        # should be green before you start
```
...
- **Tests pass.** Run `python -m pytest friday/tests/ -q` before opening a PR; add
  focused tests for new behavior under `friday/tests/test_<area>.py`. Tests are
  offline/mocked and need no API key.
- **Add tools the v2 way.** Drop a `friday/tools/<name>.py` with a
...
- **Don't hard-code the assistant's name.** It's configurable via
  ...
  `friday.config.assistant_name()` (backend) or `config.assistant_name` (UI). Only
  `FRIDAY_*` env-var names are fixed.
...
- **Keep secrets out of git.** Configuration goes in `friday/config.yaml`; secrets
