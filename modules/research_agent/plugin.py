...
class ResearchAgentPlugin(FridayPlugin):
    ...
    def on_load(self):
        # Wire research cancellation into the interrupt bus so "cancel"
        # spoken during research aborts the running thread.
        try:
            from core.interrupt_bus import get_interrupt_bus  # noqa: PLC0415
            bus = get_interrupt_bus()
            bus.subscribe("all", lambda sig: self.service.cancel() if "barge" in sig.reason or "cancel" in sig.reason else None)
        except Exception:
            logger.debug("[research] interrupt-bus wiring skipped")
    ...
    def handle_research(self, text, args):
        ...
        # Step 5d (2026-05-24) — when the intent parser has already
        # detected an explicit depth ("quick research on X", "deep
        # dive on Y"), skip the planner's "any specific angle?" prompt
        # and kick off research immediately in the right mode. Without
        # this, even "tldr X" would prompt the user for a focus +
        # depth, which defeats the point of saying "tldr".
        explicit_mode = (args.get("mode") or "").lower().strip()
        if explicit_mode in {"quick", "deep"}:
            planner = self._get_planner()
            if planner is not None and getattr(self.app.router, "session_id", None):
                return planner.begin(
                    topic,
                    self.app.router.session_id,
                    mode=explicit_mode,
                )
