...
def _find_gws() -> str | None:
    """Locate the gws binary; returns its path or None."""
    found = shutil.which("gws") or shutil.which("gws.exe")
    if found:
        return found

    _is_windows = platform.system() == "Windows"
    if _is_windows:
        # Windows: check common Go/scoop/winget install locations
        common_paths = [
            os.path.expanduser("~/go/bin/gws.exe"),
            os.path.join(os.environ.get("LOCALAPPDATA", ""), "Programs", "gws", "gws.exe"),
            os.path.join(os.environ.get("APPDATA", ""), "gws", "gws.exe"),
            os.path.join(os.environ.get("ProgramFiles", "C:\\Program Files"), "gws", "gws.exe"),
            os.path.join(os.environ.get("USERPROFILE", ""), "scoop", "apps", "gws", "current", "gws.exe"),
        ]
    else:
        common_paths = [
            ...
            "/usr/local/bin/gws",
            "/usr/bin/gws",
            "/opt/homebrew/bin/gws",   # macOS Homebrew
            "/home/linuxbrew/.linuxbrew/bin/gws",  # Linux Homebrew
            "/snap/bin/gws",
        ]
...
def _run(*args: str, timeout: int = 20) -> Any:
    ...
    if gws_path is None:
        if platform.system() == "Windows":
            # On Windows, bash -lc is unlikely to work; fail with a clear message.
            raise GWSError(
                "gws CLI not found. Install from https://github.com/googleworkspace/cli "
                "or place gws.exe on your PATH."
            )
    ...
    # bash exit 127 means "command not found" (Linux/macOS only)
...
PRIMARY_UNREAD_QUERY = "is:unread category:primary"


def gmail_list_unread(max_results: int = 10, query: str = PRIMARY_UNREAD_QUERY) -> list[dict]:
    """Return unread messages from `gws gmail +triage`.

    Defaults to the **Primary** category only (`is:unread category:primary`)
    so "check my mail" surfaces the inbox that matters, not Promotions/Social
    bulk. Pass an explicit ``query`` (e.g. ``"is:unread"``) to widen it.
