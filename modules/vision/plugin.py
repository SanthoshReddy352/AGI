...
class VisionPlugin(FridayPlugin):
    ...
    def on_load(self) -> None:
        ...
        # P3.14 — describe a file or URL image (always enabled)
        self.app.register_capability(
            {
                "name": "describe_image",
                "description": (
                    "Describe or analyze an image from a file path or URL. "
                    "Pass a local file path or a public image URL."
                ),
                "parameters": {"path_or_url": "string — local file path or public image URL"},
                "aliases": [
                    "describe this image", "describe image", "analyze this image",
                    "what is in this image", "what does this image show",
                    "describe the image at", "analyze the photo", "what's in this photo",
                ],
                "patterns": [
                    r"\bdescribe\s+(?:this\s+|the\s+)?(?:image|photo|picture)\b",
                    r"\banalyze\s+(?:this\s+|the\s+)?(?:image|photo|picture)\b",
                    r"\bwhat(?:'s|\s+is)\s+in\s+(?:this\s+|the\s+)?(?:image|photo)\b",
                ],
                "context_terms": [
                    "describe image", "analyze photo", "what is in this image",
                    "image path", "describe the picture",
                ],
    ...
    def _handle_describe_image(self, raw_text: str, args: dict):
        """P3.14 — describe an image by file path or URL."""
        import re as _re
        path_or_url = args.get("path_or_url") or args.get("path") or args.get("url", "")
        if not path_or_url:
            m = _re.search(r"(https?://\S+|/[\w/.\-]+\.(png|jpg|jpeg|gif|webp|bmp))", raw_text, _re.IGNORECASE)
            path_or_url = m.group(0) if m else ""
        if not path_or_url:
            return "Please provide a file path or URL for the image you want described."
        self._ack("Describing that image…")
        try:
            image = self._load_image(path_or_url)
            if image is None:
                return f"Couldn't load image from: {path_or_url}"
            from modules.vision import prompts
            result = self._service.infer(image, prompts.DESCRIBE_IMAGE, max_tokens=150)
            return self._ok("describe_image", result or "The model loaded the image but didn't produce a description.")
        except Exception as exc:
            return self._err("describe_image", exc)

    def _load_image(self, path_or_url: str):
        """Return an image object (PIL Image or bytes) from a local path or URL."""
