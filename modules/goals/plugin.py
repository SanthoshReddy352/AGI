...
class GoalsPlugin(FridayPlugin):
    ...
    def on_load(self):
        ...
        self.app.register_capability({
            "name": "delete_goal",
            "description": "Permanently delete a goal and its progress history.",
            "parameters": {"goal_id": "string – goal ID to delete"},
        }, self.handle_delete)

        self.app.register_capability({
            "name": "select_goal_candidate",
            "description": "Pick a goal from a pending disambiguation list. Internal use.",
            "parameters": {"selection": "string – number, ordinal, or title"},
    ...
    # NOTE: the real handle_update (with title disambiguation) lives further
    # down — a duplicate definition without disambiguation used to shadow it
    # here and was removed (2026-05-31). Keep a single source of truth.

    def handle_list(self, text: str, args: dict) -> str:
        status = args.get("status") or "active"
        goals = self._memory().list_goals(status=status)
        if not goals:
    ...
    def _find_goals_by_title(self, title: str, status: str = "active") -> list:
        """Return all active goals whose title contains the given text."""
        goals = self._memory().list_goals(status=status)
        return [g for g in goals if title.lower() in g["title"].lower()]

    def _disambiguate_or_return(self, title: str, tool: str, args: dict) -> str | None:
        """When multiple goals match, set pending selection and return None
        (caller should return the disambiguation message). Otherwise return
        the single goal ID or None."""
        matches = self._find_goals_by_title(title)
        if not matches:
            return None
        if len(matches) == 1:
            return matches[0]["id"]
        ds = getattr(self.app, "dialog_state", None)
        if ds:
            ds.set_pending_goal_selection(matches, tool, original_args=args)
        return "__disambiguate__"

    def _format_goal_list(self, goals: list) -> str:
        lines = [f"Multiple goals match ({len(goals)}):"]
        for i, g in enumerate(goals, 1):
            pct = int(g.get("score", 0) * 100)
            lines.append(f"  {i}. [{g['level']}] {g['title']} — {pct}%")
        lines.append("")
        lines.append("Which one? Say the number or name.")
        return "\n".join(lines)
    ...
    def handle_update(self, text: str, args: dict) -> str:
        ...
        if not goal_id and not title:
            return "Please specify a goal_id or title."
        if not goal_id and title:
            resolved = self._disambiguate_or_return(title, "update_goal", args)
            if resolved == "__disambiguate__":
                return self._format_goal_list(self._find_goals_by_title(title))
            if resolved is None:
                return f"No active goal found matching '{title}'."
            goal_id = resolved
    ...
    def handle_complete(self, text: str, args: dict) -> str:
        ...
        if not goal_id:
            title = args.get("title", "").strip()
            if title:
                resolved = self._disambiguate_or_return(title, "complete_goal", args)
                if resolved == "__disambiguate__":
                    return self._format_goal_list(self._find_goals_by_title(title))
                if resolved is None:
                    return f"No active goal found matching '{title}'."
    ...
    def handle_delete(self, text: str, args: dict) -> str:
        goal_id = args.get("goal_id", "").strip()
        title = args.get("title", "").strip()
        if not goal_id and title:
            resolved = self._disambiguate_or_return(title, "delete_goal", args)
            if resolved == "__disambiguate__":
                return self._format_goal_list(self._find_goals_by_title(title))
            if resolved is None:
                return f"No active goal found matching '{title}'."
            goal_id = resolved
        if not goal_id and not title:
            active = self._memory().list_goals(status="active")
            if not active:
                return "No active goals to delete."
            ds = getattr(self.app, "dialog_state", None)
            if ds:
                ds.set_pending_goal_selection(active, "delete_goal", {})
            return self._format_goal_list(active)
        if not goal_id:
            return "Please specify a goal_id."
        goal = self._memory().get_goal(goal_id)
        if not goal:
        ...
        if guard is not None and guard.needs_confirmation(args):
            return guard.arm(
                action="delete_goal", args={"goal_id": goal_id},
                preview=f"I'll delete the goal '{goal['title']}'.",
            )
        self._memory().delete_goal(goal_id)
        return f"Goal '{goal['title']}' deleted."

    def handle_select_goal(self, text: str, args: dict) -> str:
        ds = getattr(self.app, "dialog_state", None)
        if not ds or not ds.has_pending_goal_selection():
            return "No pending goal selection."
        pending = ds.pending_goal_selection
        selection = (args.get("selection") or text or "").strip().lower()
        if selection in {"cancel", "never mind", "forget it", "skip", "none"}:
            ds.clear_pending_goal_selection()
            return "Cancelled."
        from modules.system_control.file_search import choose_candidate_from_text
        titles = [g["title"] for g in pending.candidates]
        picked, error = choose_candidate_from_text(selection, titles)
        if error:
            return error
        if picked is None:
            exact = [g for g in pending.candidates if selection in g["title"].lower()]
            if len(exact) == 1:
                picked = exact[0]["title"]
            else:
                return f"Could not match '{selection}'. Say the number or name."
        goal = next(g for g in pending.candidates if g["title"] == picked)
        goal_id = goal["id"]
        tool = pending.tool
        ds.clear_pending_goal_selection()
        handler_map = {
            "update_goal": self.handle_update,
            "complete_goal": self.handle_complete,
            "delete_goal": self.handle_delete,
        }
        handler = handler_map.get(tool)
        if not handler:
            return f"Unknown action '{tool}'."
        new_args = dict(pending.original_args)
        new_args["goal_id"] = goal_id
        return handler(text, new_args)

    def _morning(self, payload: dict) -> None:
        goals = self._memory().list_goals(status="active")
