...
def take_screenshot():
    ...
    Priority:
      ...
      2. xdg-desktop-portal  (portal — needs gi; works on KDE, blocked by GNOME)
      3. GNOME Shell D-Bus via gdbus CLI (no gi dependency)
      4. GNOME Shell D-Bus via PyGObject
      5. gnome-screenshot adapter  (watches ~/Pictures/Screenshots/)
      6. grim                (wlroots compositors: sway, Hyprland)
      7. spectacle           (KDE Plasma)
      8. generic X11 tools   (xfce4-screenshooter, maim, scrot, import)
      9. pyautogui           (X11/Windows fallback)
    """
    ...
    # Debug logging — when every backend fails, log the chain so the user
    # can paste it into the issue tracker. Old behavior collapsed
    # everything to "needs python3-gi" which was actively misleading.
    def _record(backend: str, err: str) -> None:
        errors.append(f"{backend}: {err}")
        logger.debug("[screenshot] %s failed: %s", backend, err)
    ...
    if os.name != "nt":
        ...
        if is_wayland:
            ...
            # 2. xdg-desktop-portal — generic portal interface. Works on
            #    KDE/xdg-desktop-portal-kde; on GNOME the portal blocks
            #    non-privileged callers ("Access Denied"), so this is a
            #    secondary fallback after Mutter ScreenCast.
            ...
            # 3. GNOME Shell D-Bus via gdbus CLI (no gi)
            ...
            # 4. GNOME Shell D-Bus via PyGObject (if gi + gir1.2-glib-2.0)
    ...
    # Honest reporting: spell out *which* backend failed with *what* error
    # instead of guessing "python3-gi missing" off a substring match. The
    # old detector tripped on "GStreamer unavailable" / "PyGObject
    # unavailable" too, claiming python3-gi was missing when it wasn't.
    # Only flag python3-gi when we genuinely saw a `gi` import failure.
    gi_import_failed = any(
        ("no module named 'gi'" in e.lower()) or ("modulenotfounderror: gi" in e.lower())
        for e in errors
    )
    if gi_import_failed:
        msg = (
            "Screenshot requires the python3-gi bindings. Install with: "
            "sudo apt install python3-gi gir1.2-glib-2.0 gir1.2-gst-plugins-base-1.0 "
            "gstreamer1.0-pipewire xdg-desktop-portal-gnome"
        )
    else:
        details = " | ".join(err for err in errors if err) or "no backend succeeded"
        msg = f"Screenshot failed across every backend: {details}"
    logger.error(msg)
    return msg
...
def _take_screenshot_via_mutter_screencast(filepath):
    ...
    try:
        ...
        # 5s is plenty — Mutter ScreenCast typically completes in <1s.
        # Was 15s, which dominated the user's "screenshot timed out" UX.
...
def _take_screenshot_via_portal(filepath, timeout_seconds=8, interactive=False):
    """
