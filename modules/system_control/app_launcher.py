...
def find_app_candidates(app_name):
    """Return distinct canonical apps an *ambiguous* name could mean.

    Used by the disambiguation guard: when the user says "open chrom" it could
    mean Chrome or Chromium. Returns the sorted list of distinct canonical
    names whose alias begins with the (normalized) input. Returns ``[]`` when
    the name is exact (unambiguous) or matches a single canonical — the caller
    then launches normally. Only fires for inputs of ≥2 chars so a stray "c"
    doesn't fan out to every app.
    """
    normalized = normalize_app_name(app_name).strip(" .,!?")
    if len(normalized) < 2:
        return []
    registry = get_app_registry()
    # An exact alias hit is unambiguous, even if other aliases share a prefix.
    if normalized in registry:
        return []
    canonicals = []
    for alias, target in registry.items():
        if alias.startswith(normalized) and target.canonical_name not in canonicals:
            canonicals.append(target.canonical_name)
    return sorted(canonicals) if len(canonicals) > 1 else []
