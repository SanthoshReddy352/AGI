...
class FileManageRequest:
    ...
    # Track 1.5: when True, `content` came verbatim from the user
    # (quoted-string in the utterance) and must be written literally —
    # never passed through `_generate_requested_content`. Without this
    # flag, "write 'Hello Friday' to hello.txt" would treat 'Hello Friday'
    # as a topic prompt and the LLM would invent an essay.
    content_is_literal: bool = False
...
class WorkspaceFileController:
    ...
    def confirm_yes(self, text, args):
        ...
        if pending_clarification and pending_clarification.action_text:
            ...
            # Track 3.2b: was `router.process_text(...)` — v1 entry.
            # `dispatch_sub_text` routes the saved action through the
            # v2-native intent → capability path without entering a new
            # turn (confirm_yes is already inside an active turn).
            dispatch = getattr(self.app, "dispatch_sub_text", None)
            if callable(dispatch):
                return dispatch(pending_clarification.action_text)
    ...
    def manage(self, text, args):
        ...
        if (
            ...
            and not request.content_is_literal
    ...
    def _publish_explicit_artifact(self, target_path: str) -> None:
        """Save the working artifact for a just-mutated file.

        Track 1.2 (Consolidation Direction): scope is `last_write`, not
        `explicit`. A successful file mutation (create/write/append/rename)
        must always supersede any stale earlier pronoun target so that
        "read it" after the write returns the just-written file rather than
        a stale "open X" anchor from a few turns back. `last_write` outranks
        `explicit` in `ARTIFACT_SCOPE_RANK`.
        """
        memory = self._memory()
        ...
        if memory is None or not session_id or not hasattr(memory, "save_artifact"):
            return
        try:
            from core.stores import WorkingArtifact  # noqa: PLC0415
            ...
            logger.debug("[file_controller] last_write artifact save skipped: %s", exc)
    ...
    def parse_manage_request(self, text, args=None):
        ...
        # Track 1.5: detect quoted content in the original utterance so we
        # can suppress the content-generator and write the user's text
        # verbatim. Track 1.4b moved the wrap-style check into the shared
        # `slot_extractors` module.
        from core.planning.slot_extractors import content_is_quoted_in  # noqa: PLC0415
        content_is_literal = bool(content) and (
            bool(args.get("content"))
            or content_is_quoted_in(text, content)
        )
        use_last_assistant_message = self._wants_last_assistant_message(text_lower, content)
        if use_last_assistant_message:
            content = ""
            content_is_literal = False
        ...
        return FileManageRequest(
            ...
            content_is_literal=content_is_literal,
        )
    ...
    def _extract_manage_content(self, text):
        # Track 1.4b: the extraction logic lives in
        # `core/planning/slot_extractors.py` so ContextResolver (and any
        # future content-bearing handler) can call the same function. This
        # method stays as a thin wrapper for callers that already hold a
        # WorkspaceFileController reference.
        from core.planning.slot_extractors import extract_quoted_content  # noqa: PLC0415
        return extract_quoted_content(text)
