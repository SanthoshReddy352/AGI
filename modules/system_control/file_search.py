...
_ORDINAL_WORDS_TO_INDEX = {
    "first": 0, "second": 1, "third": 2, "fourth": 3, "fifth": 4,
    "sixth": 5, "seventh": 6, "eighth": 7, "ninth": 8, "tenth": 9,
}
# Track 1.3: digit-form ordinals stripped of suffix (1st → 1).
...
def choose_candidate_from_text(selection_text, candidates):
    ...
    # Track 1.3: collapse "the first one" / "1st one" / "first item" / etc.
    # to the bare ordinal token so the existing index path handles it.
    head = cleaned.split()
    if head and head[0] == "the":
        head = head[1:]
    if head and len(head) >= 2 and head[1] in {"one", "option", "item", "file", "result"}:
        head = head[:1]
    leading = head[0] if head else ""

    if leading == "last":
        return candidates[-1], None
    if leading in _ORDINAL_WORDS_TO_INDEX:
        index = _ORDINAL_WORDS_TO_INDEX[leading]
        if 0 <= index < len(candidates):
            return candidates[index], None
        return None, "That number is outside the available options."
    digit_match = _DIGIT_ORDINAL_RE.fullmatch(leading)
    if digit_match:
