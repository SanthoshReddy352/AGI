...
class STTEngine:
    def __init__(self, app_core):
        ...
        # --- Noise-robustness knobs (signal-to-noise pipeline) ---
        # Greedy decode is fast/accurate in a quiet room; beam search wins when
        # the utterance is noisy. We pick per-utterance from an SNR estimate.
        self.beam_size = self._config_int("voice.stt_beam_size", 1, "FRIDAY_WHISPER_BEAM_SIZE")
        self.beam_size_noisy = self._config_int("voice.stt_beam_size_noisy", 5, "FRIDAY_WHISPER_BEAM_SIZE_NOISY")
        self.snr_noisy_db = self._config_float("voice.stt_snr_noisy_db", 15.0)
        self.vad_filter = self._config_bool("voice.stt_vad_filter", True, "FRIDAY_WHISPER_VAD_FILTER")
        self.domain_prompt = self._config_str("voice.stt_domain_prompt", "", "FRIDAY_WHISPER_PROMPT") or None
        self.normalize_audio = self._config_bool("voice.stt_normalize_audio", True, "FRIDAY_WHISPER_NORMALIZE")
        self.denoise_enabled = self._config_bool("voice.stt_denoise", False, "FRIDAY_WHISPER_DENOISE")
        self._denoise_unavailable = False  # set True after a failed noisereduce import
        ...
        # On Windows (no hardware echo cancellation) default to wake-word-only
        # barge-in so speaker output doesn't interrupt TTS. On Linux the user
        # typically has a headset or PulseAudio echo cancellation, so the
        # more responsive RMS barge-in is the better default.
        _platform_default_barge_in = platform.system() == "Windows"
        self.barge_in_require_wake_word = self._config_bool(
            "voice.barge_in_require_wake_word", _platform_default_barge_in,
            "FRIDAY_BARGE_IN_REQUIRE_WAKE_WORD"
        )
        # RMS barge-in is disabled when wake-word-only mode is active so speaker
        # echo never stops TTS before the transcript check can reject it.
        self.use_rms_barge_in = not self.barge_in_require_wake_word
        ...
        # In speaker mode (barge_in_require_wake_word) the echo can arrive
        # up to max_utterance_duration (~20s) after TTS stops; extend window.
        _echo_window_default = 30.0 if self.barge_in_require_wake_word else 1.8
        self.assistant_echo_window_s = self._config_float("conversation.assistant_echo_window_s", _echo_window_default)
    ...
    def _listen_loop(self):
        ...
        try:
            ...
            with sd.InputStream(
                ...
                _prev_tts_speaking = False

                while self._loop_active:
                    # In speaker/wake-word mode: when TTS stops, discard the audio
                    # buffer that accumulated during playback — it's all echo.
                    if self.barge_in_require_wake_word:
                        _tts_now = self._tts_is_actively_speaking()
                        if _prev_tts_speaking and not _tts_now and audio_buffer:
                            audio_buffer = []
                            silence_frames = 0
                            empty_polls = 0
                            self._drop_audio_until = time.monotonic() + 0.5
                        _prev_tts_speaking = _tts_now
    ...
    def _process_voice_text(self, text):
        ...
        # Only treat wake_found as a genuine invocation when "friday" appears
        # at the utterance start. A day-name mid-sentence ("Today is Friday")
        # should not bypass echo detection.
        _genuine_wake = wake_found and bool(
            re.match(r'^\s*(?:hey\s+)?(?:friday|florida|freddy|fry\s*day)', text_clean, re.I)
        )
        if self._looks_like_recent_assistant_echo(text_clean) and not _genuine_wake and not self.app_core.is_speaking:
        ...
        if self._tts_is_actively_speaking():
            ...
            if self.barge_in_require_wake_word:
                # Strict mode: only "friday" (or a wake-word variant) interrupts.
                # _looks_like_fresh_command is skipped — speaker echo can match it.
                if not is_friday:
                    self._reject_transcript("speaker echo during speech (wake word required)", text_clean, log_level="debug")
                    return
            else:
                # Default: speaker (non-BT) rejects non-friday, non-command echo.
            ...
            if stop_word_matched or is_friday or (not self.barge_in_require_wake_word and self._looks_like_fresh_command(text_clean)):
    ...
    def _estimate_snr_db(self, audio_data):
        """Estimate signal-to-noise ratio (dB) from the captured utterance.

        We slice the audio into ~30 ms windows and compare a high-percentile
        window RMS (speech) against a low-percentile window RMS (noise floor).
        Cheap (numpy only) and self-contained — no model needed. Returns a
        large value when the buffer is too short to judge so we default to the
        fast greedy path.
        """
        audio = np.asarray(audio_data, dtype=np.float32).reshape(-1)
        win = max(1, int(self.target_samplerate * 0.03))
        n_win = len(audio) // win
        if n_win < 4:
            return 99.0
        ...
        return float(20.0 * np.log10(signal / noise)) if noise > 0 else 99.0

    def _build_transcribe_kwargs(self, snr_db):
        """Assemble faster-whisper kwargs, adapting effort to the noise level.

        Quiet utterances stay on greedy decode (lowest latency). Noisy ones
        get beam search plus temperature fallback for accuracy. Hallucination
        thresholds reject noise-only segments at the model level.
        """
        noisy = snr_db < self.snr_noisy_db
        kwargs = {
            "beam_size": self.beam_size_noisy if noisy else self.beam_size,
            "best_of": self.beam_size_noisy if noisy else self.beam_size,
            "patience": 1,
            "condition_on_previous_text": False,
            "vad_filter": self.vad_filter,
            # Reject hallucinated transcripts on near-silent / noise-only audio.
            "no_speech_threshold": 0.6,
            "log_prob_threshold": -1.0,
            "compression_ratio_threshold": 2.4,
            # Temperature fallback only kicks in when decoding looks degenerate;
            # harmless (and skipped) for clean greedy decodes.
            "temperature": [0.0, 0.2, 0.4] if noisy else 0.0,
        }
        if self.vad_filter:
            kwargs["vad_parameters"] = {"min_silence_duration_ms": 300, "speech_pad_ms": 200}
        if self.domain_prompt:
            kwargs["initial_prompt"] = self.domain_prompt
    ...
    def _transcribe_buffer(self, audio_buffer):
        ...
        snr_db = self._estimate_snr_db(audio_data)
        transcribe_kwargs = self._build_transcribe_kwargs(snr_db)
        logger.debug(
            "[Whisper] Transcribing (SNR~%.1fdB, beam=%s, vad=%s)...",
            snr_db, transcribe_kwargs["beam_size"], transcribe_kwargs["vad_filter"],
        )
    ...
    def _prepare_audio_for_transcription(self, audio_data):
        ...
        if self.normalize_audio:
            audio_data = self._normalize_level(audio_data)
        return np.asarray(audio_data, dtype=np.float32)

    def _normalize_level(self, audio_data):
        """Remove DC offset and RMS-normalize toward a target level.

        Lifts a quiet speaker above the room noise floor without the spectral
        artifacts of subtraction-based denoisers. Gain is clamped so we never
        blow up a near-silent (noise-only) buffer into loud hiss.
        """
        if len(audio_data) == 0:
            return audio_data
        audio_data = audio_data - float(np.mean(audio_data))  # DC removal
        rms = float(np.sqrt(np.mean(audio_data ** 2)))
        if rms < 1e-5:
            return audio_data
        gain = min(8.0, max(0.5, 0.1 / rms))  # target RMS ~0.1, clamped
        return np.clip(audio_data * gain, -1.0, 1.0)

    def _denoise(self, audio_data):
        """Optional spectral-subtraction denoise via the `noisereduce` package.

        Off by default (``voice.stt_denoise``). Import is lazy and failure is
        non-fatal: if the package is missing we log once and fall through with
        the original audio so the pipeline keeps working.
        """
        if self._denoise_unavailable or len(audio_data) == 0:
            return audio_data
        try:
            import noisereduce as nr  # noqa: PLC0415
            return nr.reduce_noise(
                y=audio_data, sr=self.target_samplerate, stationary=True
            ).astype(np.float32)
        except ImportError:
            self._denoise_unavailable = True
            logger.warning("[STT] stt_denoise enabled but `noisereduce` not installed; skipping.")
            return audio_data
        except Exception as exc:
            logger.debug("[STT] denoise skipped: %s", exc)
            return audio_data

    def transcribe_audio_file(self, path: str) -> str:
        """Transcribe an audio file (ogg, m4a, wav, mp3, …) to text.

        Uses ffmpeg subprocess to decode to 16 kHz mono PCM, then passes the
        numpy array to the same faster-whisper model used for live voice.
        Returns empty string on failure.
        """
        if self.model is None:
            logger.warning("[STT] transcribe_audio_file: model not loaded")
            return ""
        import subprocess
        import tempfile
        try:
            ...
            result = subprocess.run(
                ["ffmpeg", "-y", "-i", path, "-ar", "16000", "-ac", "1",
                 "-f", "s16le", "-acodec", "pcm_s16le", tmp_path],
                capture_output=True, timeout=30,
            )
            if result.returncode != 0:
                logger.warning("[STT] ffmpeg decode failed: %s", result.stderr.decode()[:200])
                return ""
            audio_data = np.fromfile(tmp_path, dtype=np.int16).astype(np.float32) / 32768.0
            import os as _os
            _os.unlink(tmp_path)
        except Exception as exc:
            logger.warning("[STT] transcribe_audio_file decode error: %s", exc)
            return ""
        try:
            ...
            logger.warning("[STT] transcribe_audio_file transcription error: %s", exc)
            return ""
