...
def apply_input_device_selection(target):
    ...
    # A dict without a recognised kind (or with no id) means "no explicit
    # selection" — fall back to the system default rather than handing a dict
    # to sounddevice as a device handle.
    if isinstance(target, dict):
        if target.get("id") in (None, ""):
            return {"device": None, "label": "System default"}
        return {"device": target.get("id"), "label": target.get("label") or str(target.get("id"))}
