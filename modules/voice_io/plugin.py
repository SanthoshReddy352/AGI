...
from .voice_mode import make_voice_mode_controller
...
class VoiceIOPlugin(FridayPlugin):
    def __init__(self, app):
        ...
        # P3.19: voice mode (TTS mute/unmute, timed mute). Persists to
        # data/runtime_state.json so a restart respects the last decision.
        self.voice_mode = make_voice_mode_controller(app)
        app.voice_mode = self.voice_mode
    ...
    def on_load(self):
        ...
        self.app.register_capability({
            "name": "voice_mode",
            "description": (
                "Mute or unmute spoken responses. Use to silence FRIDAY's "
                "voice without disabling the microphone."
            ),
            "parameters": {
                "state": "string - 'mute' / 'unmute' / 'on' / 'off' / 'text_only'",
                "duration_minutes": "integer (optional) - timed mute window",
            },
            "context_terms": [
                "mute yourself", "be quiet", "stop talking",
                "text only mode", "text-only mode",
                "speak again", "unmute yourself", "talk again",
                "be quiet for a minute", "be quiet for five minutes",
            ],
    ...
    def handle_voice_mode(self, text=None, args=None):
        args = dict(args or {})
        state = str(args.get("state") or "").strip().lower()
        duration = args.get("duration_minutes")
        raw_text = str(text or "").lower()
        if not state:
            if any(p in raw_text for p in ("speak again", "unmute", "talk again", "voice on")):
                state = "unmute"
            elif any(p in raw_text for p in ("mute", "be quiet", "text only", "text-only", "stop talking")):
                state = "mute"
            else:
                state = "mute"
        if duration is None:
            # Parse "for N minutes" out of the raw utterance.
            m = re.search(r"for\s+(\d+)\s*(?:minute|minutes|min|mins)", raw_text)
            if m:
                duration = int(m.group(1))
        try:
            return self.voice_mode.set(state, duration_minutes=duration)
        except Exception as exc:
            logger.warning("[VoiceIO] voice_mode error: %s", exc)
            return "I couldn't change the voice mode."
