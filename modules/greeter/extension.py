...
# Track 5.2b: onboarding is a YAML template, not a Python BaseWorkflow.
# The template name must match `core/workflows/templates/user_onboarding.yaml`.
ONBOARDING_WORKFLOW_NAME = "user_onboarding"
...
class GreeterExtension(Extension):
    ...
    @capability(
        name="identify_self",
        description=(
            "Return a canned identity for FRIDAY. Use when the user asks who/what FRIDAY is, "
            "what its name is, or asks it to introduce itself. NEVER routes to the LLM — keeps "
            "the system prompt from leaking via a small chat model echo."
        ),
    )
    def handle_identify_self(self):
        term = self._address_term()
        lines = [
            f"I'm FRIDAY — a local-first AI assistant running entirely on your machine, {term}.",
            f"FRIDAY at your service, {term}. I'm a local desktop assistant — I handle apps, files, "
            "reminders, the web, and general questions, all without sending your data to the cloud.",
            f"Name's FRIDAY, {term}. I'm your personal AI assistant — local, voice-driven, and ready "
    ...
    def handle_startup(self):
        ...
        hasn't been marked completed, kick off the `user_onboarding` YAML
        template's slot-fill loop and return the first question as the
        spoken greeting. The next user turn lands in the template's
        slot-fill resume path via `WorkflowOrchestrator.continue_active`.
        """
        context_store = self.ctx.get_service("context_store")

        # First-run onboarding takes priority over any other startup path.
        if self._should_start_onboarding(context_store):
            question = self._begin_onboarding()
            if question:
                logger.info("[greeter] First-run onboarding triggered: %s", question)
                return question
            # If we couldn't begin (no orchestrator/session), fall through to
            # the normal greeting — at worst the user sees "sir" and can
            # answer questions later via update_user_profile.
    ...
    def _begin_onboarding(self) -> str:
        """Kick off the YAML `user_onboarding` template's slot-fill loop so
        the next user turn lands in :meth:`TemplateWorkflow.run_slot_fill_turn`
        via :meth:`WorkflowOrchestrator.continue_active`.

        Returns the first question (the spoken greeting that begins the
        onboarding) on success, or an empty string when the orchestrator /
        session aren't available — callers fall back to a normal greeting.
        """
        orchestrator = self.ctx.get_service("workflow_orchestrator")
        session_id = self.ctx.get_service("session_id")
        if orchestrator is None or not session_id:
            logger.warning(
                "[greeter] Cannot start onboarding — orchestrator=%s session_id=%s",
                bool(orchestrator), bool(session_id),
            )
            return ""
        try:
            result = orchestrator.start_template_slot_fill(
                ONBOARDING_WORKFLOW_NAME, session_id,
            )
        except Exception as exc:
            logger.warning("[greeter] Failed to start onboarding template: %s", exc)
            return ""
        if not result.handled:
            logger.warning(
                "[greeter] Onboarding template did not start: %s", result.response,
            )
            return ""
