...
# Startup speed: when the Hugging Face models are already cached locally,
# skip the Hub's network round-trips (the "unauthenticated requests to the
# HF Hub" warning + the slow "Loading weights" revision checks) by switching
# the HF stack to offline mode. This shaves several seconds off cold start —
# important because a wake word launches FRIDAY and the user waits for it.
# Guards: honour an explicit choice (either var already set), let the user
# force online with FRIDAY_HF_ONLINE=1, and only go offline if a HF cache
# actually exists (so a first-ever run can still download).
try:
    import os as _os2
    if _os2.environ.get("FRIDAY_HF_ONLINE") != "1" and "HF_HUB_OFFLINE" not in _os2.environ:
        _hf_home = _os2.environ.get("HF_HOME") or _os2.path.expanduser("~/.cache/huggingface")
        if _os2.path.isdir(_os2.path.join(_hf_home, "hub")):
            _os2.environ["HF_HUB_OFFLINE"] = "1"
            _os2.environ["TRANSFORMERS_OFFLINE"] = "1"
    # Quiet the tokenizers fork warning that can also stall first output.
    _os2.environ.setdefault("TOKENIZERS_PARALLELISM", "false")
except Exception:
    pass
...
def _install_signal_handlers(kernel: RuntimeKernel) -> None:
    ...
    Uses os._exit(0) after shutdown to force process termination — avoids
    the "force quit" dialog that Qt shows when SystemExit is raised while
...
def main():
    ...
    # Build the greeter payload eagerly (cheap string) so the HUD/CLI can
    # fire it the moment its event-bus subscribers are wired — no sleep needed.
    greeter = app.extension_loader.get_extension("Greeter")
    greeting = greeter.handle_startup() if greeter else None
    ...
    start_hud(app, greeting=greeting)
