...
def test_record_turn_ignores_legacy_extractor_kwarg():
    """Track 2.3 removed the Mem0 extractor. The `extractor` constructor
    kwarg is accepted-and-ignored for one release so out-of-tree callers
    don't crash; record_turn no longer fans out to it. Ambient extraction
    now runs through MemoryBroker.curate, exercised by
    test_memory_facade tests."""
    store = MagicMock()
    extractor = MagicMock()  # constructor accepts but ignores
    svc = MemoryService(store, extractor=extractor)
    svc.record_turn("sess", "what's the weather", "sunny")
    extractor.queue_turn.assert_not_called()
