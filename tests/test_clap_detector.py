...
def test_launch_friday_uses_project_venv_and_main_entrypoint():
    ...
    # Detached spawn is platform-specific: POSIX uses start_new_session,
    # Windows uses DETACHED_PROCESS creationflags (see launch_friday).
...
def test_detector_process_match_ignores_shell_wrappers():
    command = "python3 -c import subprocess; subprocess.Popen(['/opt/friday/modules/voice_io/clap_detector.py'])"

    assert _detector_process_matches(command) is False


def test_detector_process_match_accepts_real_script_invocation():
    command = "/usr/bin/python3 /opt/friday/modules/voice_io/clap_detector.py --start"
