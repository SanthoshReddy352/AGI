...
def app_with_router():
    ...
    # Force the direct-call research path. MagicMock returns a non-None
    # child for `workflow_orchestrator` / `workflows.get(...)`, which makes
    # handle_research delegate to a phantom ResearchPlannerWorkflow and
    # never reach `_announce_completion`. The completion-emission tests
    # depend on the direct path firing on_complete synchronously.
    app.workflow_orchestrator = None
    # MagicMock attributes default to truthy MagicMocks. handle_research
    # treats `telegram_turn_active` as a flag and `_announce_completion`
    # branches on `comms` — both default to truthy here, which would route
    # the announce path to a phantom Telegram channel and skip the assistant
    # emit. Pin them to falsy so the direct emit path is the only one live.
