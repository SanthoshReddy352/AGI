...
def test_missing_wake_model_uses_transcript_fallback(monkeypatch):
    # No Picovoice key -> Porcupine wake engine unavailable -> transcript fallback.
...
def test_missing_wake_model_can_report_rejected_reason_when_fallback_disabled(monkeypatch):
    monkeypatch.delenv("FRIDAY_PORCUPINE_KEY", raising=False)
    app = MagicMock()
    app.is_speaking = False
    app.media_control_mode = False
    ...
    # Wake engine unavailable for a concrete, surfaced reason (no key here).
    assert stt.last_rejected_reason == "FRIDAY_PORCUPINE_KEY not set"
    assert stt.wake_armed is False
