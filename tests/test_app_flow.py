...
# Track 3.2 (Consolidation Direction): v1 dispatch retired. Tests that
# previously mocked `app.router.process_text` now mock
# `app.turn_manager.handle_turn` — the canonical v2 entry — instead.
# The `route_text` (cleaned) argument is asserted from there.


def test_process_input_emits_user_and_assistant_messages():
    app = FridayApp()
    app.turn_manager.handle_turn = MagicMock(return_value="Hi from FRIDAY")
    callback = MagicMock()
...
def test_process_input_normalizes_typed_commands_before_routing():
    ...
    # handle_turn receives the cleaned text as its first positional arg.
    call = app.turn_manager.handle_turn.call_args
    assert call.args[0] == "open calculator"


def test_typed_input_interrupts_active_speech_before_processing():
    app = FridayApp()
    app.turn_manager.handle_turn = MagicMock(return_value="Opening chrome.")
    app.tts = MagicMock()
    ...
    call = app.turn_manager.handle_turn.call_args
    assert call.args[0] == "open chrome"


def test_voice_input_does_not_auto_stop_tts_in_process_input():
    app = FridayApp()
    app.turn_manager.handle_turn = MagicMock(return_value="On it.")
    app.tts = MagicMock()
    app.is_speaking = True
