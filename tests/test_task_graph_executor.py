...
def test_dependency_output_injected_into_dependent_args():
    ...
    # Filter internal injection keys (e.g. __step_id__ for observation stashing,
    # added by Phase 5) — these are infra, not user-facing args.
    def _user_args(d: dict) -> dict:
        return {k: v for k, v in d.items() if not k.startswith("__")}

    assert _user_args(seen_args["search"]) == {}
    # Downstream sees its own args plus the upstream's output keyed by node_id.
    assert _user_args(seen_args["summarize"]) == {"hint": "be brief", "search": "search-output"}
