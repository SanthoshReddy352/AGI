...
def test_query_document_fires_when_active_document_present():
    """A WH-question routes to query_document when the session has an
    active document set in the reference store. Track 1.4c removed the
    `[active_document=...]` text-injection prefix; the parser now reads
    ...
    router.context_store.get_reference = lambda sid, key: (
        "/home/user/report.pdf" if key == "active_document" else ""
    )
    router.session_id = "sess-1"
    recognizer = IntentRecognizer(router)

    plans = recognizer.plan("what does it say about the budget?")
...
def test_query_document_does_not_fire_without_active_document():
    """A plain WH-question with no active document set must NOT route to
    query_document — generic questions still reach the LLM/chat fallback."""
    from core.intent_recognizer import IntentRecognizer
    router = MagicMock()
    router.dialog_state = DialogState()
    router._tools_by_name = {"query_document": object()}
    router.context_store = MagicMock()
    router.context_store.get_reference = lambda sid, key: ""  # nothing stored
    router.session_id = "sess-1"
    recognizer = IntentRecognizer(router)
