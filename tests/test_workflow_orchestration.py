...
def build_test_app(tmp_path):
    ...
    # Lightweight executors so YAML-template slot-fill workflows (e.g. the
    # set_reminder template, §5.4 Step 3) can run their `extract_with` steps
    # and capability steps by dispatching through the router's registered
    # tools — the production app wires the real CapabilityExecutor /
    # OrderedToolExecutor here.
    app.capability_executor = _RouterCapabilityExecutor(app.router)
    app.ordered_tool_executor = _RouterOrderedExecutor(app.router)
    return app


class _RouterCapabilityExecutor:
    """Dispatch a single capability by name through the router's tool table.
    Mirrors CapabilityExecutor.execute(name, text, args) for tests."""

    def __init__(self, router):
        self.router = router

    def execute(self, name, text, args=None):
        tool = self.router._tools_by_name.get(name)
        if not tool:
            return ""
        return tool["callback"](text or "", dict(args or {}))


class _RouterOrderedExecutor:
    """Run a compiled ToolPlan's steps in order through the router's tool
    table, returning the last step's response. Mirrors the production
    OrderedToolExecutor closely enough for template slot-fill tests."""
    ...
    def execute(self, plan, user_text, turn=None):
        ...
        for step in plan.steps:
            tool = self.router._tools_by_name.get(step.capability_name)
            if tool is not None:
                response = tool["callback"](user_text or "", dict(step.args or {}))
        return response
...
def test_reminder_prompts_for_missing_date_and_time(monkeypatch, tmp_path):
    ...
    # §5.4 Step 3: reminder slot-fill is now the `set_reminder` YAML template
    # (ReminderWorkflow retired). With neither date nor time on the first turn
    # the template asks for the date first (two-phase date→time preserved).
    assert prompt == "What date should I remind you?"
    state = app.context_store.get_active_workflow(app.session_id, workflow_name="set_reminder")
    assert state["awaiting_slot"] == "date"
    assert state["slots"]["message"] == "purchase a gift"
...
def test_reminder_followup_date_then_time_completes_event(monkeypatch, tmp_path):
    ...
    # Two-phase date→time fill, now driven by the set_reminder template.
    assert first == "What date should I remind you?"
...
def test_reminder_accepts_bare_hour_when_context_expects_time(monkeypatch, tmp_path):
    ...
    # Seed the date on the first turn ("today"), so the template asks only for
    # the time; a bare "four" on the time question reads as 4 o'clock, and the
    # ambiguous past-morning hour bumps to the afternoon (4:00 PM). (The first
    # turn carries "today" so it isn't a standalone date word that the get_date
    # intent would otherwise poach mid-workflow.)
    second = app.router.process_text("remind me to purchase a gift today")
...
def test_reminder_accepts_at_hour_in_first_sentence(monkeypatch, tmp_path):
    class FixedDatetime(datetime):
        @classmethod
        def now(cls):
            return cls(2026, 4, 28, 15, 36, 0)
    ...
    # Date + bare hour both in the first sentence → resolved immediately
    # (afternoon inference), no follow-up turn.
    result = app.router.process_text("remind me to purchase a gift today at 4")
...
def test_reminder_with_date_and_time_in_first_sentence_has_no_followup(monkeypatch, tmp_path):
    ...
    # Complete on the first turn → scheduled immediately, no slot-fill template
    # left active.
    state = app.context_store.get_active_workflow(app.session_id, workflow_name="set_reminder")
    assert not state
...
def test_reminder_confirmation_uses_remind_wording(monkeypatch, tmp_path):
    ...
    # §5.4 Step 3: _handle_reminder_parts retired; the shared scheduling helper
    # _schedule_reminder owns the confirmation wording now.
    result = plugin._schedule_reminder("call John", FUTURE)
...
def test_briefing_lists_reminders_only(monkeypatch, tmp_path):
    # Local calendar events were removed (2026-05-31); the briefing now lists
    # reminders only. Legacy calendar_event rows are ignored.
    monkeypatch.setattr(task_manager_plugin, "datetime", FixedNow)
    monkeypatch.setattr(task_manager_plugin, "DB_PATH", str(tmp_path / "friday.db"))
    monkeypatch.setattr(TaskManagerPlugin, "_schedule_system_notification", lambda self, *a: False)
    app = build_test_app(tmp_path)
